...
exports.createApiProcessorManager = function(baseDebugPort, loggingConfig)
{
    ...
    var apiManager =
    {
		createApiProcessorAsync: function(appPath, appContainer, services, runForked, enableDebug, onCompleted)
    	{
            ...
            {
                logger.info("Creating managed API processor for appPath: " + appPath);
            }

            var apiProcessor = require("./lib/api-request-delegator")(appContainer, services.sessionStoreSpec, services.moduleStoreSpec, services.resourceResolverSpec, runForked, debugPort, loggingConfig, function(err, apiProcessor)
            {
                if (!err)
                {
        ...
        getAppModuleStore: function(appPath)
        {
            ...
            if (apiProcessor)
            {
                if (apiProcessor.appModuleStore == null)
                {
                    // This first time we check, there won't be a moduleStore, so we create it...
                    //
                    var moduleStore = exports.createServiceFromSpec(apiProcessor.moduleStoreSpec);
                    apiProcessor.appModuleStore = moduleStore.getAppModuleStore(apiProcessor.appContainer)
                }

                return apiProcessor.appModuleStore;
            }
