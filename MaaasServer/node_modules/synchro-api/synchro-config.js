...
exports.getConfig = function(configFile, overrides)
{
	...
	//
	nconf.use("memory");
	var memStore = nconf.stores["memory"];
	...
	}).forEach(function (key) 
	{
		...
		//
	    memStore.set(key.split("__").slice(1).join(":"), process.env[key]);
	});
	...
	if (configFile)
	{
		nconf.configDetails = "Using configuration file specified on command line: " + configFile;
		nconf.file({ file: configFile });
	}
	else if (process.env['SYNCHRO_CONFIG'])
	{
		nconf.configDetails = "Using configuration file specified in SYNCHRO_CONFIG: " + process.env['SYNCHRO_CONFIG'];
		nconf.file({ file: process.env['SYNCHRO_CONFIG'] });		
	}
	else
	{
		nconf.configDetails = "Using default configuration file: config.json";
