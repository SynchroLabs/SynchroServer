...
describe("API Processor", function()
{
	...
	describe("Initial Page Request", function ()
	{
		...
		before(function* ()
		{
			...
			session = yield sessionStore.createSession();
        ...
        {
			yield apiProcessor.process(session, requestObject, response);
        ...
        {
			yield apiProcessor.process(session, requestObject, response);
			yield readerWriter.readAsync(session.id + ":0", function * (err, responseObject)
			{
	...
	describe("Navigation", function()
	{
		...
		{
			it("should be false on top-level page with no OnBack handler", function * () 
			{
				apiProcessor = createApiProcessor(testModules);
		    	Synchro = require("../lib/app-services")(apiProcessor, null);
	        ...
	        it("should be true when back stack supports pop and no OnBack", function * () 
	        {
	        	yield startAtMenu();
	...
	describe("View model change notification", function ()
	{
		...
		it("should call OnViewModelChange with correct changes and source when client sends view model changes only", function * () 
		{
			...
			yield readerWriter.readAsync(session.id + ":1", function * (err, responseObject)
			{
				assert.equal(responseObject.Error, undefined, "Unexpected error: " + util.formatJSON(responseObject.Error));
	...
	describe("Partial ViewModel updates", function ()
	{
		...
		before(function * ()
		{
			var modules =
			{
				...
				countdown:
				{
					...
					{
					    yield Synchro.waitFor(context, Synchro.waitInterval, 100);
					    viewModel.count = 3;
					...
					Commands:
					{
					    countdown: function * (context, session, viewModel)
					    {
					        ...
					        {
					            yield Synchro.waitFor(context, Synchro.waitInterval, 50);
					            viewModel.count--;
        ...
        it("should give partial response after InitializeViewModel, followed by complete response after LoadViewMode", function * () 
        {
			var requestObject = 
			{ 
				Mode: "Page", 
			...
			function * onResponse(err, responseObject)
			{
				...
				switch (plan.count)
				{
					case 0:
					{
						...
						yield readerWriter.readAsync(session.id + ":0", onResponse);
					}
					break;
			...
			yield readerWriter.readAsync(session.id + ":0", onResponse);
			yield apiProcessor.process(session, requestObject, response);
			yield apiProcessor.process(session, nextRequest, {});
		...
		it("should give partial response after InitializeViewModel, followed by complete response after LoadViewModel, when navigated to", function * () 
        {
			...
			function * onResponse(err, responseObject)
			{
				...
				switch (plan.count)
				{
					...
					case 1:
					{
						...
						nextRequest = responseObject.NextRequest;
						yield readerWriter.readAsync(session.id + ":2", onResponse);
					}
					break;
			...
			yield readerWriter.readAsync(session.id + ":1", onResponse);
			yield apiProcessor.process(session, menuRequestObject, response);
			...
			yield readerWriter.readAsync(session.id + ":2", onResponse);
			yield apiProcessor.process(session, countdownRequestObject, response);
			yield apiProcessor.process(session, nextRequest, {});
        ...
        it("should return multiple partial responses, followed by complete response, on command using Synchro.interimUpdate", function * () 
        {
    		// Need to read session back from store (in-memory local copy doesn't contain changes)
    		session = yield sessionStore.getSession(session.id);
			...
			function * onResponse(err, responseObject)
			{
				...
				switch (plan.count)
				{
					case 0:
					{
						...
						nextRequest = responseObject.NextRequest;
						yield readerWriter.readAsync(session.id + ":3", onResponse);
					}
					break;
        ...
        it("should record view model and session updates from before and after a call to Synchro.waitFor()", function * ()
        {
			...
			yield readerWriter.readAsync(session.id + ":4", function * (err, responseObject)
		    {
	...
	describe("Sync Errors", function()
	{
		it("should fail with sync error when server has no active instance (session corrupt)", function * ()
		{
			...
			var session = yield sessionStore.createSession();
			var metrics = devices.setSessionDeviceAndViewMetrics({}, "iPhone4");
	...
	describe("Resync", function ()
	{
		...
		it("should provide only view model on resync when instance id is current", function * ()
		{
			...
			yield readerWriter.readAsync(session.id + ":2", function * (err, responseObject)
			{
				assert.equal(responseObject.Error, undefined, "Unexpected error: " + util.formatJSON(responseObject.Error));
	...
	describe("services method parameter validation", function ()
	{
		...
		var modules =
		{
			test:
			{
				...
				Commands:
				{
				    ...
				    {
				    	yield Synchro.waitFor();
				    },
				    testInterimUpdate: function * (context)
				    {
				    	yield Synchro.interimUpdate();
				    },
		...
		it("should fail if invalid context passed to waitFor", function * () 
		{
    ...
    describe("Version Errors", function()
    {
        ...
        function * testVersion(serverClientVersionSpec, appClientVersionSpec, clientVersion, expectedResponse)
        {
            var appDefinition =
            {
                name: "Test App",
        ...
        {
            yield testVersion(null, null, null, expectedSuccessResponse);
            test.assertDone();
        });
        ...
        {
            yield testVersion("1.1.0", null, "1.1.0", expectedSuccessResponse);
            test.assertDone();
        });
        
        it("should pass with client version specified and satisfying app version requirement", function * ()
        {
            yield testVersion(null, "1.1.0", "1.1.0", expectedSuccessResponse);
            test.assertDone();
        });
        ...
        it("should fail with server version required and no client version specified", function * ()
        {
            ...
            yield testVersion("1.1.0", null, null, expectedResponse);
            test.assertDone();
        });
        ...
        it("should fail with server version required and failing client version specified", function * ()
        {
            ...
            yield testVersion("1.1.0", null, "1.0.0", expectedResponse);
            test.assertDone();
        });
        ...
        it("should fail with app version required and no client version specified", function * ()
        {
            ...
            yield testVersion(null, "1.1.0", null, expectedResponse);
            test.assertDone();
        });
        ...
        it("should fail with app version required and failing client version specified", function * ()
        {
            ...
            yield testVersion(null, "1.1.0", "1.0.0", expectedResponse);
            test.assertDone();
        });
    });
});
