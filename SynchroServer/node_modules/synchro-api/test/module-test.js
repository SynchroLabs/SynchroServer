...
describe("Synchro API module", function () 
{
	...
	describe("Processor Manager", function() 
	{
		...
		it("Should return module store for API processor", function * ()
		{
            ...
            var inprocModuleStore = yield apiProcessorManager.getAppModuleStore("inproc2", "testapp", moduleStoreSpec);
            var forkedModuleStore = yield apiProcessorManager.getAppModuleStore("forked2", "testapp", moduleStoreSpec);
            
            assert.notEqual(inprocModuleStore, null);
            assert.notEqual(forkedModuleStore, null);
			assert.equal(yield apiProcessorManager.getAppModuleStoreForPath("inproc2"), inprocModuleStore); 
			assert.equal(yield apiProcessorManager.getAppModuleStoreForPath("forked2"), forkedModuleStore); 
		});
	...
	describe("Built-in Services", function() 
	{
		it("Should create file module store", function * () 
		{
			...
			var appModuleStore = yield service.getAppModuleStore("testapp");
		...
		it("Should create memory session store", function * () 
		{
			...
			var sessionId = yield service.createSession();
