...
describe("Synchro API module", function () 
{
	describe("Service creation", function () 
	{
		it("Should create test service from spec", function * () 
		{
			...
			var service = yield synchroApiModule.createServiceFromSpecAwaitable(serviceSpec);
	...
	{
        var conf = synchroConfig.loadConfigFromObject({
		...
		{
			inprocProcessor = yield apiProcessorManager.createApiProcessorAwaitable("inproc", false, false);
			assert.notEqual(inprocProcessor, null);
			assert.equal(inprocProcessor.isForked, false);
		});

		it("Should create specified forked API processor", function * () 
		{
			forkedProcessor = yield apiProcessorManager.createApiProcessorAwaitable("forked", true, true);
		...
		it("Should fail to create API processor if one already exists at the specified path", function * (done) 
		{
            try
            {
                var apiProcessor = yield apiProcessorManager.createApiProcessorAsync("inproc", false, false);
		...
		{
            var inprocModuleStore = yield apiProcessorManager.getAppModuleStoreAwaitable("inproc2", "testapp");
            var forkedModuleStore = yield apiProcessorManager.getAppModuleStoreAwaitable("forked2", "testapp");
        ...
        after(function()
        {
            logger.info("Shut down forked processor");
            forkedProcessor.shutdown();
        });
    });
	...
	describe("Built-in Services", function() 
	{
		...
		{
			var service = yield synchroApiModule.createServiceAwaitable("FileModuleStore", { directory: __dirname } );
		...
		{
			var service = yield synchroApiModule.createServiceAwaitable("MemorySessionStore", { } );
		...
		it("Should create resource resolver", function * () 
		{
			...
			var service = yield synchroApiModule.createServiceAwaitable("ResourceResolver", { prefix: "/test" } );
			assert.equal(service.getResourceUrl(context, "foo"), "http://somehost.com/test/foo");
		});
