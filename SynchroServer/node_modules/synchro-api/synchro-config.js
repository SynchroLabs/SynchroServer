...
//
//    pkgCloudModuleStore // Supports Azure, AWS, OpenStack/BlueMix, GCloud, others.
//        provider
//        [rest of config as required by provider]
//
//    RedisSessionStore
//        host
//        port
...
function getExternalIPAddress()
{
    ...
    {
        // logger.debug("Could not determine external IP address", e);
    }
...
// Get config object from raw config data object
//
exports.loadConfigFromObject = function(obj)
{
...
exports.getConfig = function(configFile, overrides)
{
	...
	else if (process.env['SYNCHRO_CONFIG'])
	{
		...
		conf.file('config.json', process.env['SYNCHRO_CONFIG']);
	}
	else
	{
    ...
    var studioUsers = conf.get("STUDIO_USERS");
    var usersDefined = studioUsers && (typeof studioUsers === 'object') && (Object.keys(studioUsers).length > 0);

    // 4) Module store apps configuration (if any).  If this exists, it will be mutually exlusive to an APPS setting in
    //    the config.json store above (if that value is null, or a string, it indicates that the apps config should come
    //    from a file in the module store).
    //
    conf.use('module_store_apps', {type: 'memory'});

	// 5) Default values
	//
    var defaults = 
    {
        // NO_SYNCDEPS - Unset (false) by default - used to prevent dependency sync (including npm install) to local file system
        //
        // APP_RESOURCE_PREFIX - Can be specified to override resource location, server-wide (must be fully qualified)
        //
        // SERVER_URL_BASE - Protocol + host + port (if non-default port) - When resolving the default resource prefix,
        //                   this value will be used if present, otherwise the host header from the request will be used
        //
        'STATIC_CACHE_MAXAGE': 86400000, // One day
        'CLIENT_VERSION': ">=1.2",
        'PROTOCOL': 'http',
        ...
        'STUDIO_NOAUTH': !usersDefined, // We will use default auth if there are any users defined
        'STUDIO_TITLE': "Synchro API Server", 
        'STUDIO_SUBTITLE': "The following Synchro apps are being served here:",
