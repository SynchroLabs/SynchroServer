...
exports.createApiProcessorManager = function(baseDebugPort, config)
{
    ...
    var apiManager =
    {
		createApiProcessorAsync: function(appPath, runForked, enableDebug, onCompleted)
    	{
            ...
            var apiProcessor = require("./lib/api-request-delegator")(appPath, config, runForked, debugPort, function(err, apiProcessor)
            {
                if (!err)
                {
        ...
        getAppModuleStoreAwaitable: function * (appPath, appContainer, moduleStoreSpec) // Generator
        {
        ...
        getAppModuleStoreForPathAwaitable: function * (appPath) // Generator
        {
            ...
            if (!moduleStores[appPath])
            {
                ...
                if (apiProcessor)
                {
                    yield this.getAppModuleStoreAwaitable(appPath, apiProcessor.appContainer, apiProcessor.moduleStoreSpec);
                }
                else
                {
...
exports.createService = function(serviceName, serviceConfiguration)
{
    ...
    switch (serviceName)
    {
        ...
        {
            service = fileModuleStore(serviceConfiguration);
        }
        break;

        case "MantaModuleStore":
        {
            service = mantaModuleStore(serviceConfiguration);
        }
        break;

        case "MemorySessionStore":
        {
            service = memorySessionStore(serviceConfiguration);
        }
        break;

        case "FileSessionStore":
        {
            service = fileSessionStore(serviceConfiguration);
        }
        break;

        case "PkgCloudModuleStore":
        {
            service = pkgCloudModuleStore(serviceConfiguration);
        }
        break;

        case "RedisSessionStore":
        {
            service = redisSessionStore(serviceConfiguration);
        }
        break;

        case "ResourceResolver":
        {
            service = resourceResolver(serviceConfiguration);
        }
        break;
    }
