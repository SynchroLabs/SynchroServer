...
function createForkedRequestProcessorAsync(appPath, config, debugPort, onCreated)
{
    ...
    childProcess.on('message', function(message) 
    {
        switch (message.type)
        {
            ...
            case "reloadModule":
            {
                logger.info("Processing pending reload id: " + message.id);
                var pendingReload = pendingReloads[message.id];
                delete pendingReloads[message.id];
                logger.info("Pending reload message: ", message);
...
function * createInProcRequestProcessorAsync(appPath, config, onCreated) // Generator
{
    try
    {
        var apiRequestProcessorModule = require("./api-request-processor");
        var apiRequestProcessor = yield apiRequestProcessorModule.createApiRequestProcessorAsync(appPath, config);
        ...
        var requestProcessor =
        {
            ...
            {
                return yield apiRequestProcessor.reloadModule(moduleName);
            }
        }
        onCreated(null, requestProcessor);
    }
...
module.exports = function(appPath, config, fork, debugPort, onCreated)
{
    ...
    {
        co(createInProcRequestProcessorAsync, appPath, config, onCreated).catch(function(err)
        {
