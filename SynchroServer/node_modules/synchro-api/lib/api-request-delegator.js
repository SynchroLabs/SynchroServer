...
function createForkedRequestProcessorAsync(appPath, config, debugPort, onCreated)
{
    ...
    var args = [appPath]; // args passed to child process (will start at argv[2] in child process)
    var options = {};
    if (debugPort)
    {
    ...
    // We now send the config via a message to the child process instead of passing it on the cmdline (so we
    // don't leak secrets, for one thing).  This "config" message it what actually triggers the processor to 
    // get created and send back the status message of "started".
    //
    logger.debug("Sending config to child process");
    childProcess.send({cmd: "config", config: JSON.stringify(config.get())});
    ...
    var asyncOpsManager = new AsyncOperationsManager(30000); // !!! Should be configurable
    ...
    childProcess.on('message', function(message) 
    {
        switch (message.type)
        {
            ...
            case "httpRequest":
            {
                ...
                var pendingRequest = asyncOpsManager.operationComplete(message.id); // Returns value if non timed-out
                if (pendingRequest)
                {
