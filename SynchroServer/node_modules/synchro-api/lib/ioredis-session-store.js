    ...
    function * createClientAwaitable()
    {
        ...
        if (params.sentinels)
        {
            if (params.sentinels === true)
            {
                ...
                {
                    logger.debug("Getting Sentinels from Consul, attempt %d", i);
                    ...
                    {
                        logger.debug("Failed to get sentinels from Consul, waiting %d ms before retry", wait);
                        yield function(done){setTimeout(done, wait)};
                    }
                }

                if (!clientOpts.sentinels)
                {
                    throw new Error("Failed to get Sentinels from Consul after %d attempts, failed to create client", maxTries);
                }
            }
