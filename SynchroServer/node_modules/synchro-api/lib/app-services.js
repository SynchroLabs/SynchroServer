    ...
    var services = 
    {
        getResourceUrl: function(context, resource)
        {
            if (resource)
            {
                ...
                {
                    throw new Error("A valid context must be the first parameter to Synchro.getResourceUrl()");
                }
                return resourceResolver.getResourceUrl(context, resource);
            }
            else
            {
                // No context, first param is actually the resource
                return resourceResolver.getResourceUrl(null, context);
            }
        },
        ...
        storeStateBeforeYieldAwaitable: function * (context)
        {
            if (!apiProcessor.isValidContext(context))
            {
                throw new Error("A valid context must be the first parameter to Synchro.storeStateBeforeYieldAwaitable()");
            }
            return yield apiProcessor.storeStateBeforeYieldAwaitable(context);
        },

        restoreStateAfterYieldAwaitable: function * (context)
        {
            if (!apiProcessor.isValidContext(context))
            {
                throw new Error("A valid context must be the first parameter to Synchro.restoreStateAfterYieldAwaitable()");
            }
            return yield apiProcessor.restoreStateAfterYieldAwaitable(context);
        },

        yieldAwaitable: function * (context, yieldable)
        {
            if (!apiProcessor.isValidContext(context))
            {
                throw new Error("A valid context must be the first parameter to Synchro.yieldAwaitable()");
            }
            return yield apiProcessor.yieldAwaitable(context, yieldable);
        },
        ...
        getSessionId: function(context)
        {
            if (!apiProcessor.isValidContext(context))
            {
                throw new Error("A valid context must be the first parameter to Synchro.getSessionId()");
            }
