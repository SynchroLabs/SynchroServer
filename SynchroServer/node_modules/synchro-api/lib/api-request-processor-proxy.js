...
function reloadModule(id, moduleName)
{
    var result = apiRequestProcessor.reloadModule(moduleName);
    logger.info("Loaded module: ", moduleName, result);
    process.send({type: "reloadModule", id: id, result: result});
}

function createApiRequestProcessor(appPath, config)
{
	logger.debug("Initializing API request processor");
	apiRequestProcessor = apiRequestProcessorModule.createApiRequestProcessorAsync(appPath, config);
	...
	process.on('message', function(message, handle) 
	{
	    ...
	    switch (message.cmd)
	    {
	        ...
	        case "reloadModule":
	            wait.launchFiber(reloadModule, message.id, message.moduleName); //handle in a fiber
	            break;
	    }
	});
...
if (!module.parent)
{
	...
	*/
	...
	wait.launchFiber(createApiRequestProcessor, appPath, config);
}
