...
function * reloadModule(id, moduleName) // Generator
{
    var result = yield apiRequestProcessor.reloadModule(moduleName);
    logger.info("Loaded module: ", moduleName, result);
    process.send({type: "reloadModule", id: id, result: result});
}

function * createApiRequestProcessor(appPath, config) // Generator
{
	logger.debug("Initializing API request processor");
	apiRequestProcessor = yield apiRequestProcessorModule.createApiRequestProcessorAsync(appPath, config);
	...
	process.on('message', function(message, handle) 
	{
	    ...
	    switch (message.cmd)
	    {
	        ...
	        case "reloadModule":
	            co(reloadModule, message.id, message.moduleName).catch(function(err)
		        {
...
if (!module.parent)
{
	...
	*/
	...
	co(createApiRequestProcessor, appPath, config).catch(function(err)
    {
