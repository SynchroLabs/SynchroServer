...
function * reloadModuleAwaitable(id, moduleName)
{
    ...
    logger.debug("Loaded module: ", moduleName, result);
    process.send({type: "reloadModule", id: id, result: result});
}

function * checkHealth(id)
{
    var health = yield apiRequestProcessor.healthCheck();
    process.send({type: "health", id: id, result: health});
}
...
function * createApiRequestProcessorAwaitable(appPath, config)
{
	...
	process.on('message', function(message, handle) 
	{
	    ...
	    switch (message.cmd)
	    {
	        ...
	        case "healthCheck":
	            co(checkHealth, message.id).catch(function(err)
		        {
...
if (!module.parent)
{
	...
	*/
	...
	{
		// SIGINT will typically come from the terminal when shutting down the process group (on CTRL+C, for example)
		//
	    logger.info('SIGINT - shutting down child processer for:', appPath);
        process.exit();
	});

	process.on('SIGTERM', function ()
	{
		// SIGTERM will typically come from parent process when it explicitly kills this child process via child_process.kill() 
		//
	    logger.info('SIGTERM - shutting down child processer for:', appPath);
        process.exit();
	});
	...
	process.on('message', function(message, handle) 
	{
		...
		{
			var configData = JSON.parse(message.config);
			...
			logger.debug("Forked API child process started: " + filename);
