...
exports.createApiRequestProcessorAsyncAwaitable = function * (appPath, config, callback)
{
    ...
    var apiPathPrefix = config.get('API_PATH_PREFIX');

    // See if we have an app-specific resource prefix...
    //
    var resourcePrefix = config.get('APPS:' + appPath + ':APP_RESOURCE_PREFIX');
    if (!resourcePrefix)
    {
        // Get the system-wide resource prefix (if any)
        //
        resourcePrefix = config.get('APP_RESOURCE_PREFIX');
    }

    //
    // Note: If an app-specific or system-wide prefix is specified (above), it is assumed to be fully
    //       qualified and to end with a trailing slash.  This case is typical when pointing to a CDN, 
    //       a URL mapping to http-accessible cloud storage, etc.
    //

    if (!resourcePrefix)
    {
        // We did not have a fully-qualified resource prefix - resources are being served locally by Synchro
        // (from app-specific default path).
        //
        resourcePrefix = config.get('API_PATH_PREFIX') + '/' + appPath + '/resources/';
    }
    ...
    { 
        ...
        {
            serverUrlBase: config.get('SERVER_URL_BASE'),
            hostGuess: config.get("PROTOCOL") + "://" + config.addNonStandardPort(config.get('HOST')),
            prefix: resourcePrefix
        }
    }

    var sessionStore = yield synchroApi.createSessionStoreAwaitable(config);
    var moduleStore = yield synchroApi.createModuleStoreAwaitable(config);
    var resourceResolver = yield synchroApi.createServiceFromSpecAwaitable(resourceResolverSpec);
    
    var appContainer = config.get('APPS')[appPath].container;

    var moduleManager = yield require('./module-manager')(moduleStore, config, appContainer, resourceResolver);
    ...
    function * internalProcessHttpRequestAwaitable (request, callback)
    {
        ...
        // We're going to keep the "host" request header to use in places where we need to try to guess
        // at our endpoint address (like resource resolution when no fully qualified prefix exitsts).
        // Likewise, we will use the "secure" property of the request (which is set if the connection
        // to this server was SSL, and should be set by any SSL terminating proxies when they were
        // connected to by SSL) to help us guess the protocol.
        //
        requestObject.Host = request.headers['host'];
        requestObject.Secure = request.secure;
        ...
        if (requestObject.Mode === "Page")
        {
            ...
            {
                logger.debug("Incrementing module instance");
