...
exports.createApiRequestProcessorAsync = function * (appPath, config, callback)
{
    ...
    function * apiProcess (session, request, response) // Generator
    {
        return yield api.process(session, request, response); 
    }
    ...
    //
    function * internalProcessHttpRequest (request, callback) // Generator
    {
        ...
        if (sessionId)
        {
            ...
            session = yield sessionStore.getSession(sessionId);
        }
        ...
        //       ready).
        //
        yield readerWriter.readAsync(channelId, function * (err, responseObject) // Genertor (passed callback)
        {
            if (err)
            {
        ...
        {
            yield apiProcess(session, requestObject, responseObject);
        }
    }
    ...
    var publicApi =
    {
        processHttpRequest: function(request, callback)
        {
            ...
            co(internalProcessHttpRequest, request, callback).catch(function(err)
            {
                logger.error("Error in internal http process request:", err);
            });;
        },

        reloadModule: function * (moduleName) // Generator
        {
            logger.debug("API reloading module: " + moduleName);
            return yield * api.reloadModule(moduleName);
        }
    }
