...
module.exports = function * (moduleStore, appRootPath, appContainer, resourceResolver) // !!! Gonna need the server version too (from it's package.json)
{
    ...
    {
        logger.info("Watching module store for changes");
        ...
        if (appModuleStore.unwatch)
        {
            ...
            {
                // NOTE: This takes about 15-20ms on my dev machine, and is roughly the same regardless of number
                //       of modules (tried with a single file "Hello" and with SynchroSamples and they were the same).
                //       Not sure if this is going to cause us any heartburn on shutdown in production environments.
                //       If the process is going away anyway, maybe it's better to just not worry about unwatch, but
                //       that feels a little dirty.  Also, in future, other module stores might use other mechanisms
                //       for wacth/unwatch, such as Redis pub/sub, which might appreciate unwatch being called.
                //
                appModuleStore.unwatch();
                logger.info("Stopped watching module store for changes");
            }
