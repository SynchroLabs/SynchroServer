...
module.exports = function (moduleStore, appRootPath, appContainer, resourceResolver) // !!! Gonna need the server version too (from it's package.json)
{
    ...
    function loadAppRoutesAndHooks()
    {
        ...
        {
            if (isHookModule(moduleNames[i]) || isRouteModule(moduleNames[i]))
            {
                loadModule(moduleNames[i]);
            }
            else
            {
                // logger.info("Not loading supporting module explicitly (will get loaded when/if referenced by a "require");
            }
        }
    ...
    function callHookFunctionByIndex(index, fnName, args)
    {
    ...
    var moduleManager = 
    {
        ...
        callHookFunction: function(fnName)
        {
            if (fnName.indexOf('After') != 0)
            {
                for (var n = 0, len = hookModules.length; n < len; n++) // Default order is forward
                {
                    callHookFunctionByIndex(n, fnName, lodash.rest(arguments)); // Remove fnName from arguments
                }
            }
            else
            {
