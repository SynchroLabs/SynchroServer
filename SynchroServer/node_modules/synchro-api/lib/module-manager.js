...
module.exports = function * (moduleStore, appRootPath, appContainer, resourceResolver) // !!! Gonna need the server version too (from it's package.json)
{
    ...
    var appModuleStore = yield moduleStore.getAppModuleStore(appContainer);
    ...
    //
    function isHookModule(fileName)
    {
    ...
    Module._extensions['.js'] = (function (original)
    {
        return function (module, filename)
        {
            ...
            if (isContainerModule(filename))
            {
                ...
                if (moduleSource)
                {
                    logger.info("Got module source, len:", moduleSource.length);
                    ...
                    logger.info("Finished compiling:", moduleName);
                }
                else
                {
    ...
    function * loadAppRoutesAndHooks()
    {
        ...
        var moduleNames = yield appModuleStore.listModules();

        logger.info("Loading modules:", moduleNames);

        moduleFiles = moduleNames;

        // We load the source for each route / hook module as we load the module (asynchronously before we call require on the module).
        // Since those modules may require other modules in this container, we need to preload the source for all of those "other" modules
        // now (there won't be any way to asynchronously load them at the time they are required).
        //
        for (var i = 0; i < moduleNames.length; i++) 
        {
            var moduleName = moduleNames[i];
            if (!isHookModule(moduleName) && !isRouteModule(moduleName))
            {
                logger.info("Loading support module:", moduleName);
                var moduleSource = yield appModuleStore.getModuleSource(moduleName);
        ...
        for (var i = 0; i < moduleNames.length; i++) 
        {
            ...
            if (isHookModule(moduleName) || isRouteModule(moduleName))
            {
                ...
                if (isHookModule(moduleName))
                {
                    var hookPath = removeFileExtension(moduleName);
                    logger.info("Loaded hook module:", hookPath);
                    hookModules[hookModuleNames.indexOf(hookPath)] = synchroModule;
                }
                else if (isRouteModule(moduleName))
                {
                    var routePath = path.basename(moduleName, path.extname(moduleName));
                    logger.info("Loaded route module:", routePath);
                    routes[routePath] = synchroModule;
                }
            }
        }
        
        // The module loading should have drained the cache (particularly of util modules).  If there is a util module that
        // was not consumed ("required") by any other module, it could still be hanging around, so we flush the source cache.
        //
    ...
    function * callHookFunctionByIndex(index, fnName, args) // Generator
    {
    ...
    var moduleManager = 
    {
        ...
        callHookFunction: function * (fnName) // Generator
        {
            if (fnName.indexOf('After') != 0)
            {
                for (var n = 0, len = hookModules.length; n < len; n++) // Default order is forward
                {
                    yield callHookFunctionByIndex(n, fnName, lodash.rest(arguments)); // Remove fnName from arguments
                }
            }
            else
            {
        ...
        loadRoutes: function * (apiProcessor)
        {
            ...
            return yield loadAppRoutesAndHooks();
        },

        reloadModule: function * (moduleName, source)
        {
            logger.info("Reload module: ", moduleName);

            if (source)
            {
                yield appModuleStore.putModuleSource(moduleName, source);
            }
            ...
            for (var cachedModule in require.cache) 
            {
                if (require.cache.hasOwnProperty(cachedModule)) 
                {
                    ...
                    {
                        logger.info("Uncaching module: " + cachedModule);
