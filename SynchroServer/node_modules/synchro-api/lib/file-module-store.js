...
exports.createServiceAwaitable = function * (params)
{
    ...
    logger.debug("Using file module store, directory", modulesDir);
    ...
    var moduleStore = 
    {
        ...
        {
            var filepath = path.resolve(modulesDir, filename);
            try
            {
                var content = util.removeBOM(fs.readFileSync(filepath, 'utf8'));
                return content;
            }
            catch (err)
            {
        ...
        getAppModuleStoreAwaitable: function * (container)
        {
            ...
            var app =
            {
                watch: function(callback)
                {
                    ...
                    watch.watchTree(appContainerPath, options, function (f, curr, prev) 
                    {
                        if (curr != null)
                        {
                            logger.info("File changed in watched directory:", f);
                            callback(f);
                        }
                    });
                },
                ...
                listModulesAwaitable: function * ()
                {
                    ...
                    yield walkAwaitable(appContainerPath);
