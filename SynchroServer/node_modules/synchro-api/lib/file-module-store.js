...
module.exports = function(params)
{
    ...
    function getAppDefinition(container)
    {
        if (container && (container.length > 0))
        {
            ...
            {
                // Defined behavior is return null if no appDefinition, so just log at debug
                logger.debug("getAppDefinition failed - no package.json in container: " + container);
                return null;
            }
        }
        else
    ...
    var moduleStore = 
    {
        ...
        createAppContainer: function(container)
        {
            ...
            else
            {
                ...
                wait.for(fs.writeFile, path.resolve(appContainerPath, "package.json"), JSON.stringify(packageJson));
            }
        },
        ...
        getAppModuleStore: function(container)
        {
            ...
            var app =
            {
                ...
                listModules: function()
                {
                    ...
                    function walk(dir)
                    {
                        var list = wait.for(fs.readdir, dir);
                        list.forEach(function(file) 
                        {
                            if (path.basename(file) != "node_modules")
                            {
                                file = path.resolve(dir, file);
                                var stat = wait.for(fs.stat, file);
                ...
                {
                    var moduleFilePath = path.resolve(appContainerPath, toLocalPath(moduleFilename));
                    wait.for(ensurePathExistsAsync, moduleFilePath);
