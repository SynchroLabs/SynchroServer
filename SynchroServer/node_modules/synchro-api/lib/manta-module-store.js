...
module.exports = function(params)
{
    ...
    // Note: This is going to return a buffer.  For cases where it's really a string (like a js or json file), an
    //       implicit toString will work (as it should be utf-8 encoded).
    //
    function * getBlobTextAwaitable(path)
    {
        try
        {
            ...
            if (result && result.length)
            {
                ...
                // If we set the encoding, we get string chunks, else we get binary Buffers
                //
    ...
    // Note: We switched from using "createWriteStream" to "put" since only the latter supports mkdirs.
    //
    ...
    var moduleStore = 
    {
        ...
        {
            var filepath = path.posix.join(modulesDir, filename); 
            return yield getBlobTextAwaitable(filepath);
        },

        putStoreFileAwaitable: function * (filename, content)
        {
            var filepath = path.posix.join(modulesDir, filename); 
            yield setBlobTextAwaitable(filepath, content);
        },
