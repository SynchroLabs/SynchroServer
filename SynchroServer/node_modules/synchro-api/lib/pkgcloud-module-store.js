...
module.exports = function(params)
{
    ...
    function * getBlobTextAwaitable(containerName, blobName)
    {
        ...
        var stream = client.download(options, function (err, res) 
        {
            if (err) 
            {
                logger.error("Error downloading container: %s, blob: %s, err: %s", containerName, blobName, err);
            }
            else
            {
                logger.debug("Done downloading container: %s, blob: %s", containerName, blobName);                
            }
        });
        ...
        getAppModuleStoreAwaitable: function * (containerName)
        {
            ...
            var app =
            {
                ...
                listModulesAwaitable: function * ()
                {
                    ...
                    var files = lodash.pluck(modules, "name");
