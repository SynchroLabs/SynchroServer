...
//     ModuleInstance
//         path
//         instanceId <- This is what we consider the "current module instance"
//         viewIsDynamic
//         viewHash (present if viewIsDynamic)
//         ClientViewModel <- The version of the ViewModel last sent to the client 
//             instanceVersion
//             ViewModel
//         ServerViewModel <- A version of the ViewModel saved by an async processor on this instanceId when it yielded
//             ViewModel
//
// context
//     session
//     request
//     response
//     LocalViewModel <- The ViewModel that Synchro processing works against
//         instanceId
//         ViewModel 
//
...
function UserCodeError(method, error) 
{
    ...
    this.method = method,
    this.error = error; // This erorr has the stack trace we care about
...
function * getViewModelAwaitable (synchroApi, route, routeModule, context, session, params, state)
{
    var viewModel = {};
    
    var instanceId = getCurrentModuleInstance(context);
    
    yield synchroApi.callHookFunctionAwaitable("BeforeInitializeViewModel", route, routeModule, context, session.UserData, params, state);
    
    // Only call the route InitializeViewModel and the "after" hook if we didn't navigate away from this page inside of
    // the BeforeInitializeViewModel hook call (this can happen in a case such as redirecting to a login page).
    //
    if (instanceId == getCurrentModuleInstance(context))
    {
        if (routeModule.InitializeViewModel)
        {
            viewModel = yield synchroApi.callUserModuleFunctionAwaitable(routeModule, routeModule.InitializeViewModel, context, session.UserData, params, state);
        }
        
        yield synchroApi.callHookFunctionAwaitable("AfterInitializeViewModel", route, routeModule, context, session.UserData, params, state, viewModel);
    }

    return viewModel;
}

function * getViewAwaitable (synchroApi, route, routeModule, context, session, viewModel, isViewMetricUpdate)
{
    ...
    yield synchroApi.callHookFunctionAwaitable("BeforeInitializeView", route, routeModule, context, session.UserData, viewModel, view, metrics, isViewMetricUpdate);
    
    if (routeModule.InitializeView)
    {
        view = yield synchroApi.callUserModuleFunctionAwaitable(routeModule, routeModule.InitializeView, context, session.UserData, viewModel, view, metrics, isViewMetricUpdate);
    }

    yield synchroApi.callHookFunctionAwaitable("AfterInitializeView", route, routeModule, context, session.UserData, viewModel, view, metrics, isViewMetricUpdate);
...
function isBackAvailable(context)
{
    // Determine if "back" is available based on either a back element on the View, or if not provided, whether
    // the back stack supports pop.  Report that to client.
    //
    var backStack = new BackStack(context.session)
    if (context.response.View.back !== undefined)
    {
        // View contained and explicit "back" member - override nav stack and return it...
        //
        logger.debug("Returning expicit View.back value of:", context.response.View.back);
        return context.response.View.back;
    }
    else
    {
        logger.debug("Returning back based on backStack");
        return (backStack.getSize() > 1);
    }
}

function * populateNewPageResponseAwaitable (synchroApi, route, routeModule, context, params, state)
{
    var instanceId = getCurrentModuleInstance(context);
    var viewModel = yield getViewModelAwaitable(synchroApi, route, routeModule, context, context.session, params, state);
    ...
    var view = yield getViewAwaitable(synchroApi, route, routeModule, context, context.session, viewModel);
    ...
    // This is a little weird.  We're writing the session, which includes the ClientViewModel (last ViewModel sent to the client),
    // even though we haven't written this to the client yet (which could still fail).  
    //
    // But the thing this does for us is that it blows away any ServerViewModel (which would have been for a previous instance) 
    // and commits that to the session, so that, among other things, when that yield returns (which could be at any point we
    // yield in processing this request, including when we yield to write it), that other async process will know that it is
    // obsolete.
    //
    // There is some special processing in sendUpdateAwaitable() (below) to send the full ClientViewModel as the View if the
    // ClientViewModel.instanceVersion is 0 (the case here).
    //
    // logger.info("Putting session on new page generation:", context.session);
    yield synchroApi.sessionStore.putSessionAwaitable(context.session); // !!! Async, could temp fail
    ...
    context.response.Back = isBackAvailable(context);
...
function * sendUpdateAwaitable (synchroApi, context, isInterim)
{
    ...
    if (synchroApi.readerWriter.isWritePending(channelId))
    {
        ...
        //
        // We will check tosee how long that pending write has been sitting in the queue and time it out as appropriate.
        // When a sequence of (interim/final) writes is going to be processed, the client is notified to immediately 
        // re-poll for subsequent writes until the final write is sent (this is known as long-polling).  The longest
        // amount of time a write should ever be pending is probably around one second (the latency of sending the 
        // initial or subsequent response, having it recieved, and the client re-polling.  We'll have out timeout be
        // 30 seconds, which should be more than enough.  
        //
        var age = synchroApi.readerWriter.getPendingWriteAge(channelId);
        if (age > (30 * 1000))
        {
            // Trnasaction abandoned.  Cancel the pending write and throw an exception (the exception will kill the
            // user code and prevent it from sending further writes on this channel).
            //
            synchroApi.readerWriter.cancelWrite(channelId);
            throw new Error("Client appears to have abandoned request, cancelling");
        }
        else
        {
    ...
    // Note: The yield below will not necessarily yield until the write callback is is satisfied.  If a corresponding
    //       read is pending, then this will yield until the write is completed (which includes writing to the
    //       reader).  However, if no corresponding read is pending, then this call will return after the write is
    //       posted (the actual write callback will be called asynchronously to this code path when the read is
    //       ready).
    //
    yield synchroApi.readerWriter.writeAsyncAwaitable(channelId, function * (err, writeDataAwaitable) // Genertor (passed callback)
    {
        if (err)
        {
        ...
        {
            // Indication of whether session data represents current or next instance and should be saved
            //
            ...
            context.session = yield synchroApi.sessionStore.getSessionAwaitable(context.session.id); // !!! Async, could temp fail
            ...
            if (!context.response.Error)
            {
                ...
                {
                    logger.debug("Resync - sending current view model, instanceId, and instanceVersion");
                ...
                { 
                    ...
                    else
                    {
                        ...
                        //
                        logger.debug("Update not for current or navigated to instance");
                        saveSession = false;
                        ...
                        // We know this response is for an obsolete request and the client is going to just ignore it, but we still need
                        // to send it (to satisfy the clients waiting request).  There will be no View, ViewModel, or ViewModelDeltas.
                        //
                        context.response.InstanceId = context.LocalViewModel.instanceId;
                        context.response.InstanceVersion = 0; // It's obsolete, and we don't have version
                    }
            ...
            yield writeDataAwaitable(context.response);
            ...
            if (!context.response.Error)
            {
                ...
                if (saveSession)
                {
                    logger.debug("Putting session after write");
                    // logger.info("Putting session after write:", context.session);
                    yield synchroApi.sessionStore.putSessionAwaitable(context.session); // !!! Async, could temp fail
                }
...
SynchroApi.prototype.loadAwaitable = function * ()
{
    this.appDefinition = yield this.moduleManager.loadRoutesAwaitable(this);
    return this.appDefinition;
}

SynchroApi.prototype.getAppDefinition = function()
{
    logger.debug("Returning app definition for app: %s", this.appDefinition.name);
    return this.appDefinition;
}

SynchroApi.prototype.reloadModuleAwaitable = function * (moduleName)
{
    return yield * this.moduleManager.reloadModuleAwaitable(moduleName);
}

// Determine if there is a hook (either "Before" or "After") installed on the given function.  This should be used
// when processing is required before calling the function, and there is a desire to skip that processing if there is no
// module function or hook installed that needs it.
//
SynchroApi.prototype.isFunctionHooked = function (functionName)
{
    return this.moduleManager.isFunctionHooked(functionName);
}

// Call the given hook function (all parameters required by the hook function should follow the function name parameter in
// calls to this function).
//
SynchroApi.prototype.callHookFunctionAwaitable = function * (functionName)
{
    try
    {
        yield this.moduleManager.callHookFunctionAwaitable.apply(this.moduleManager, arguments);
    }
    catch (e)
    {
        throw new UserCodeError(functionName, e);
    }
}

SynchroApi.prototype.callUserModuleFunctionAwaitable = function * (module, fn)
{
    var fname = fn.name || fn.fname || "Unknown";
    if (fn)
    {
        var args = lodash.slice(arguments, 2); // Remove module, function
        try
        {
            if (coHelper.isGeneratorFunction(fn))
            {
                return yield fn.apply(module, args);
            }
            else if (fn)
            {
        ...
        {
            throw new UserCodeError(fname, e);
        }
    }
}
...
//
SynchroApi.prototype.processAwaitable = function * (session, requestObject, responseObject)
{
    ...
    try
    {
        ...
        {
            logger.debug("ViewModel before deltas:", context.session.ModuleInstance.ClientViewModel.ViewModel);
            ...
            //
            // logger.info("Putting session after applying ViewModel deltas from client:", context.session);
            yield this.sessionStore.putSessionAwaitable(context.session); // !!! Async, could temp fail
            ...
            // If we have a view model change listener for this route, or that function is hooked, analyze changes and call as appropriate.
            //
            if (routeModule.OnViewModelChange || this.isFunctionHooked("OnViewModelChange"))
            {
                logger.debug("Processing view model changes from the client...");

                // Get the changelist
                //
                var viewModelUpdates = objectMonitor.getChangeList(null, viewModelBeforeUpdate, context.LocalViewModel.ViewModel);
                
                // The hook will get called whether or not there were any changes (whereas the module function only gets called if there were changes)
                //                
                yield this.callHookFunctionAwaitable("BeforeOnViewModelChange", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel, "view", viewModelUpdates);
                
                if (routeModule.OnViewModelChange && viewModelUpdates && (viewModelUpdates.length > 0))
                {
                    yield this.callUserModuleFunctionAwaitable(routeModule, routeModule.OnViewModelChange, context, context.session.UserData, context.LocalViewModel.ViewModel, "view", viewModelUpdates);
                }

                yield this.callHookFunctionAwaitable("AfterOnViewModelChange", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel, "view", viewModelUpdates);
            }
        }
        else
        {
        ...
        switch (requestObject.Mode)
        {
            case "Page":
            {
                ...
                yield populateNewPageResponseAwaitable(this, route, routeModule, context);
                initLocalViewModel(context);
            }
            break;
            ...
            case "Resync":
            {
                ...
                {
                    // Current view model will be assigned to response in sendUpdateAwaitable() below
                    //
                    if (!isCurrentModuleInstance(context, context.request.InstanceId))
                    {
                        ...
                        //
                        context.response.Path = route;
                        context.response.View = yield getViewAwaitable(this, route, routeModule, context, context.session, context.session.ModuleInstance.ClientViewModel.ViewModel);
                        context.response.Back = isBackAvailable(context);
                    }
                }
                else // No valid instance - client wants a page level reset (they may have awoken with no state except session-id)
                {
                    // The view model will already be set into the response, so we just need to add the view state
                    //
                    context.response.Path = route;
                    context.response.View = yield getViewAwaitable(this, route, routeModule, context, context.session, context.session.ModuleInstance.ClientViewModel.ViewModel);
                    context.response.Back = isBackAvailable(context);
            ...
            case "LoadPage":
            {
                ...
                yield this.callHookFunctionAwaitable("BeforeLoadViewModel", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel);
                if (routeModule.LoadViewModel)
                {
                    yield this.callUserModuleFunctionAwaitable(routeModule, routeModule.LoadViewModel, context, context.session.UserData, context.LocalViewModel.ViewModel);
                }
                yield this.callHookFunctionAwaitable("AfterLoadViewModel", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel);
            }
            break;

            case "Back":
            {
                yield this.callHookFunctionAwaitable("BeforeOnBack", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel);
                if (routeModule.OnBack)
                {
                    var result = yield this.callUserModuleFunctionAwaitable(routeModule, routeModule.OnBack, context, context.session.UserData, context.LocalViewModel.ViewModel); 
                    if (!result)
                    {
                        // If OnBack didn't return true, then it didn't handle the nav, so we take the default action...
                        //
                        logger.debug("OnBack didn't handle nav, doing default back nav");
                        this.pop(context);
                    }
                    else
                    {
                        logger.debug("OnBack handled nav");
                    }
                }
                else
                {
                ...
                yield this.callHookFunctionAwaitable("AfterOnBack", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel);
            }
            break;
            ...
            case "Command":
            {
                ...
                yield this.callHookFunctionAwaitable("BeforeCommand", route, routeModule, context.request.Command, context, context.session.UserData, context.LocalViewModel.ViewModel, context.request.Parameters);

                // Only process command if it exists...
                //
                if (routeModule.Commands && routeModule.Commands[context.request.Command])
                {
                    routeModule.Commands[context.request.Command].fname = "Command." + context.request.Command;
                    yield this.callUserModuleFunctionAwaitable(routeModule, routeModule.Commands[context.request.Command], context, context.session.UserData, context.LocalViewModel.ViewModel, context.request.Parameters); 

                    // If we have a view model change listener for this route, or the function is hooked, analyze changes, and call as appropriate.
                    //
                    if (routeModule.OnViewModelChange || this.isFunctionHooked("OnViewModelChange"))
                    {
                        // Get the changelist
                        ...
                        // The hook will get called whether or not there were any changes (whereas the module function only gets called if there were changes)
                        //                
                        yield this.callHookFunctionAwaitable("BeforeOnViewModelChange", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel, "command", viewModelUpdates);
                            
                        if (routeModule.OnViewModelChange && viewModelUpdates && (viewModelUpdates.length > 0))
                        {
                            yield this.callUserModuleFunctionAwaitable(routeModule, routeModule.OnViewModelChange, context, context.session.UserData, context.LocalViewModel.ViewModel, "command", viewModelUpdates); 
                        }

                        yield this.callHookFunctionAwaitable("AfterOnViewModelChange", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel, "command", viewModelUpdates);
                    }
                }
                ...
                yield this.callHookFunctionAwaitable("AfterCommand", route, routeModule, context.request.Command, context, context.session.UserData, context.LocalViewModel.ViewModel, context.request.Parameters);
            }
            break;
            ...
            case "ViewUpdate":
            {
                ...
                if (routeModule.OnViewMetricsChange || this.isFunctionHooked("OnViewMetricsChange"))
                {
                    var metrics = { DeviceMetrics: context.session.DeviceMetrics, ViewMetrics: context.session.ViewMetrics };
                        
                    yield this.callHookFunctionAwaitable("BeforeOnViewMetricsChange", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel, metrics);

                    yield this.callUserModuleFunctionAwaitable(routeModule, routeModule.OnViewMetricsChange, context, context.session.UserData, context.LocalViewModel.ViewModel, metrics);
                    ...
                    if (routeModule.OnViewModelChange || this.isFunctionHooked("OnViewModelChange"))
                    {
                        ...
                        // The hook will get called whether or not there were any changes (whereas the module function only gets called if there were changes)
                        //                
                        yield this.callHookFunctionAwaitable("BeforeOnViewModelChange", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel, "viewMetrics", viewModelUpdates);
                            
                        if (routeModule.OnViewModelChange && viewModelUpdates && (viewModelUpdates.length > 0))
                        {
                            yield this.callUserModuleFunctionAwaitable(routeModule, routeModule.OnViewModelChange, context, context.session.UserData, context.LocalViewModel.ViewModel, "viewMetrics", viewModelUpdates);
                        }

                        yield this.callHookFunctionAwaitable("AfterOnViewModelChange", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel, "viewMetrics", viewModelUpdates);
                    }

                    yield this.callHookFunctionAwaitable("AfterOnViewMetricsChange", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel, metrics);
                }
                ...
                if (isCurrentModuleInstance(context, context.LocalViewModel.instanceId))
                {
                    ...
                    if (context.session.ModuleInstance.dynamic)
                    {
                        ...
                        var view = yield getViewAwaitable(this, route, routeModule, context, context.session, context.LocalViewModel.ViewModel, true);
        ...
        if (context.exit)
        {
            // We exited (navigated back out of the app)
            //
            context.response.Exit = true;
        }
        else if (context.newPage)
        {
            //  If we navigated to (or back to) a new page, then we'll populate that new page into the response now..
            //
            var routeModule = this.moduleManager.getModule(context.newPage.route); 
            yield populateNewPageResponseAwaitable(this, context.newPage.route, routeModule, context, context.newPage.params, context.newPage.state);
        } 
    }
    catch (e)
    {
    ...
    yield sendUpdateAwaitable(this, context, false);
}
...
SynchroApi.prototype.navigateTo = function (context, route, params)
{
    ...
    if (routeModule)
    {
        if (!context.obsoleteProcessor)
        {
            ...
            context.newPage = { route: route, params: params };
        }
        else
        {
...
SynchroApi.prototype.pop = function (context)
{
    if (!context.obsoleteProcessor)
    {
        ...
        if (backStack.getSize() > 1)
        {
            ...
            context.newPage = { route: stackItem.route, params: stackItem.params, state: stackItem.state };
        }
        else
        {
            // Attempt to navigate "back" from top level page.  This means we are exiting the app.
            //
            context.exit = true;
        }
    }
...
// Exposed via Synchro.storeStateBeforeYieldAwaitable()
//
SynchroApi.prototype.storeStateBeforeYieldAwaitable = function * (context)
{
    ...
    //
    // logger.info("Putting session before yield:", context.session);
    yield this.sessionStore.putSessionAwaitable(context.session); // !!! Async, could temp fail
}

// Exposed via Synchro.restoreStateAfterYieldAwaitable()
//
SynchroApi.prototype.restoreStateAfterYieldAwaitable = function * (context)
{
    // Session data may have changed in response to another request while we were yielding (that is the only time that 
    // can happen, given the single-threaded nature of the environment).
    //
    ...
    // which they typically refer to as "session".  It is imperative that when we return from this method, the object
    // to which they have the reference is still the object to which context.session.UserData refers, otherwise local
    // changes they make to their "session" would be lost on the next storeStateBeforeYield or at the end of the user 
    // function.  This is slightly complex, as we are reloading the session itself below.
    //
    var originalUserDataObject = context.session.UserData;
    context.session = yield this.sessionStore.getSessionAwaitable(context.session.id); // !!! Async, could temp fail

    // Populate the UserData object with the new contents from the session...
    //
    util.assignNewContents(originalUserDataObject, context.session.UserData);

    // Assign the original UserData object back to the newly loaded session...
    //
    context.session.UserData = originalUserDataObject;
    ...
    if (isCurrentModuleInstance(context, context.LocalViewModel.instanceId))
    {
        ...
        // behavior, is that you can call this method passing the context in your user-code processing method
        // and your local viewModel will be updated (as appropriate) when it returns, even though you didn't pass
        // it in as a parameter to this method or otherwise explicitly update it.
        //
...
// Exposed via Synchro.yieldAwaitable()
//
// Yield to any "co" yieldable (Promise, generator/generator function, thunk, etc)
//
SynchroApi.prototype.yieldAwaitable = function * (context, yieldable)
{
    logger.debug("yieldAwaitable...");

    yield this.storeStateBeforeYieldAwaitable(context);
    var result = yield yieldable;
    yield this.restoreStateAfterYieldAwaitable(context);

    return result;
}

// Exposed via Synchro.interimUpdateAwaitable()
//
SynchroApi.prototype.interimUpdateAwaitable = function * (context)
{
    logger.debug("Interim update...");

    yield this.storeStateBeforeYieldAwaitable(context);
    yield sendUpdateAwaitable(this, context, true);
    yield this.restoreStateAfterYieldAwaitable(context);
}
