...
function * getViewModel (synchroApi, route, routeModule, context, session, params, state) // Generator
{
    viewModel = {};
    
    var instanceId = getCurrentModuleInstance(context);
    
    yield synchroApi.callHookFunction("BeforeInitializeViewModel", route, routeModule, context, session.UserData, params, state);
    ...
    if (instanceId == getCurrentModuleInstance(context))
    {
        ...
        {
            viewModel = yield synchroApi.callUserModuleFunction(routeModule, routeModule.InitializeViewModel, context, session.UserData, params, state);
        }
        
        yield synchroApi.callHookFunction("AfterInitializeViewModel", route, routeModule, context, session.UserData, params, state, viewModel);
    }

    return viewModel;
}

function * getView (synchroApi, route, routeModule, context, session, viewModel, isViewMetricUpdate) // Generator
{
    ...
    yield synchroApi.callHookFunction("BeforeInitializeView", route, routeModule, context, session.UserData, viewModel, view, metrics, isViewMetricUpdate);
    
    if (routeModule.InitializeView)
    {
        view = yield synchroApi.callUserModuleFunction(routeModule, routeModule.InitializeView, context, session.UserData, viewModel, view, metrics, isViewMetricUpdate);
    }

    yield synchroApi.callHookFunction("AfterInitializeView", route, routeModule, context, session.UserData, viewModel, view, metrics, isViewMetricUpdate);
...
function * populateNewPageResponse (synchroApi, route, routeModule, context, params, state) // Generator
{
    var instanceId = getCurrentModuleInstance(context);
    var viewModel = yield getViewModel(synchroApi, route, routeModule, context, context.session, params, state);
    ...
    var view = yield getView(synchroApi, route, routeModule, context, context.session, viewModel);
    ...
    yield synchroApi.sessionStore.putSession(context.session); // !!! Async, could temp fail
...
function * sendUpdate(synchroApi, context, isInterim) // Generator
{
    ...
    //       ready).
    //
    yield synchroApi.readerWriter.writeAsync(channelId, function * (err, writeData) // Genertor (passed callback)
    {
        if (err)
        {
        ...
        else
        {
            ...
            yield writeData(context.response);
...
{
    this.appDefinition = yield this.moduleManager.loadRoutes(this);
...
SynchroApi.prototype.reloadModule = function * (moduleName) // Generator
{
    return yield * this.moduleManager.reloadModule(moduleName);
}
...
//
SynchroApi.prototype.callHookFunction = function * (functionName) /// Generator
{
    try
    {
        yield this.moduleManager.callHookFunction.apply(this.moduleManager, arguments);
    }
    catch (e)
    {
...
SynchroApi.prototype.callUserModuleFunction = function * (module, fn)  // Generator
{
...
//
SynchroApi.prototype.process = function * (session, requestObject, responseObject) // Generator
{
    ...
    try
    {
        ...
        if (context.request.ViewModelDeltas)
        {
            ...
            //
            yield this.sessionStore.putSession(context.session); // !!! Async, could temp fail
            ...
            if (routeModule.OnViewModelChange || this.isFunctionHooked("OnViewModelChange"))
            {
                ...
                //                
                yield this.callHookFunction("BeforeOnViewModelChange", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel, "view", viewModelUpdates);
                
                if (routeModule.OnViewModelChange && viewModelUpdates && (viewModelUpdates.length > 0))
                {
                    yield this.callUserModuleFunction(routeModule, routeModule.OnViewModelChange, context, context.session.UserData, context.LocalViewModel.ViewModel, "view", viewModelUpdates);
                }

                yield this.callHookFunction("AfterOnViewModelChange", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel, "view", viewModelUpdates);
            }
        }
        else
        {
        ...
        switch (requestObject.Mode)
        {
            case "Page":
            {
                ...
                yield populateNewPageResponse(this, route, routeModule, context);
                initLocalViewModel(context);
            }
            break;
            ...
            case "Resync":
            {
                ...
                if ((context.request.InstanceId >= 0) && (context.request.InstanceVersion >= 0))
                {
                    ...
                    if (!isCurrentModuleInstance(context, context.request.InstanceId))
                    {
                        ...
                        //
                        context.response.Path = route;
                        context.response.View = yield getView(this, route, routeModule, context, context.session, context.session.ModuleInstance.ClientViewModel.ViewModel);
                        context.response.Back = isBackAvailable(context, routeModule);
                    }
                }
            ...
            case "LoadPage":
            {
                ...
                yield this.callHookFunction("BeforeLoadViewModel", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel);
                if (routeModule.LoadViewModel)
                {
                    yield this.callUserModuleFunction(routeModule, routeModule.LoadViewModel, context, context.session.UserData, context.LocalViewModel.ViewModel);
                }
                yield this.callHookFunction("AfterLoadViewModel", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel);
            }
            break;

            case "Back":
            {
                yield this.callHookFunction("BeforeOnBack", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel);
                if (routeModule.OnBack)
                {
                    yield this.callUserModuleFunction(routeModule, routeModule.OnBack, context, context.session.UserData, context.LocalViewModel.ViewModel); 
                }
                else
                {
                    // default "pop"
                ...
                yield this.callHookFunction("AfterOnBack", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel);
            }
            break;
            ...
            case "Command":
            {
                ...
                yield this.callHookFunction("BeforeCommand", route, routeModule, context.request.Command, context, context.session.UserData, context.LocalViewModel.ViewModel, context.request.Parameters);
                ...
                if (routeModule.Commands && routeModule.Commands[context.request.Command])
                {
                    ...
                    yield this.callUserModuleFunction(routeModule, routeModule.Commands[context.request.Command], context, context.session.UserData, context.LocalViewModel.ViewModel, context.request.Parameters); 
                    ...
                    if (routeModule.OnViewModelChange || this.isFunctionHooked("OnViewModelChange"))
                    {
                        ...
                        // The hook will get called whether or not there were any changes (whereas the module function only gets called if there were changes)
                        //                
                        yield this.callHookFunction("BeforeOnViewModelChange", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel, "command", viewModelUpdates);
                            
                        if (routeModule.OnViewModelChange && viewModelUpdates && (viewModelUpdates.length > 0))
                        {
                            yield this.callUserModuleFunction(routeModule, routeModule.OnViewModelChange, context, context.session.UserData, context.LocalViewModel.ViewModel, "command", viewModelUpdates); 
                        }

                        yield this.callHookFunction("AfterOnViewModelChange", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel, "command", viewModelUpdates);
                    }
                }
                ...
                yield this.callHookFunction("AfterCommand", route, routeModule, context.request.Command, context, context.session.UserData, context.LocalViewModel.ViewModel, context.request.Parameters);
            }
            break;
            ...
            case "ViewUpdate":
            {
                ...
                if (routeModule.OnViewMetricsChange || this.isFunctionHooked("OnViewMetricsChange"))
                {
                    ...
                    yield this.callHookFunction("BeforeOnViewMetricsChange", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel, metrics);

                    yield this.callUserModuleFunction(routeModule, routeModule.OnViewMetricsChange, context, context.session.UserData, context.LocalViewModel.ViewModel, metrics);
                    ...
                    if (routeModule.OnViewModelChange || this.isFunctionHooked("OnViewModelChange"))
                    {
                        ...
                        // The hook will get called whether or not there were any changes (whereas the module function only gets called if there were changes)
                        //                
                        yield this.callHookFunction("BeforeOnViewModelChange", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel, "viewMetrics", viewModelUpdates);
                            
                        if (routeModule.OnViewModelChange && viewModelUpdates && (viewModelUpdates.length > 0))
                        {
                            yield this.callUserModuleFunction(routeModule, routeModule.OnViewModelChange, context, context.session.UserData, context.LocalViewModel.ViewModel, "viewMetrics", viewModelUpdates);
                        }

                        yield this.callHookFunction("AfterOnViewModelChange", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel, "viewMetrics", viewModelUpdates);
                    }

                    yield this.callHookFunction("AfterOnViewMetricsChange", route, routeModule, context, context.session.UserData, context.LocalViewModel.ViewModel, metrics);
                }
                ...
                if (isCurrentModuleInstance(context, context.LocalViewModel.instanceId))
                {
                    ...
                    if (context.session.ModuleInstance.dynamic)
                    {
                        ...
                        var view = yield getView(this, route, routeModule, context, context.session, context.LocalViewModel.ViewModel, true);
        ...
        if (context.newPage)
        {
            ...
            yield populateNewPageResponse(this, context.newPage.route, routeModule, context, context.newPage.params, context.newPage.state);
        } 
    }
    catch (e)
    {
    ...
    yield sendUpdate(this, context, false);
}
...
//
SynchroApi.prototype.waitFor = function * (moduleObject, context, args) // Generator
{
    ...
    context.session = yield this.sessionStore.getSession(context.session.id); // !!! Async, could temp fail
...
//
SynchroApi.prototype.interimUpdate = function * (context) // Generator
{
    logger.debug("Interim update...");
    yield sendUpdate(this, context, true);
}
