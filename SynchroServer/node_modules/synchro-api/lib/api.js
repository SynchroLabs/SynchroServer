...
// Public API
//
var SynchroApi = function(moduleManager, sessionStore, readerWriter, appDefinition)
{
...
SynchroApi.prototype.process = function(session, requestObject, responseObject)
{
    ...
    {        
        ...
        //        
        var versRequired = "1.2"; // !!! From config!
        if (!semver.satisfies(context.session.DeviceMetrics.clientVersion, versRequired))
        {
            throw new ClientVersionError(context.session.DeviceMetrics.clientVersion, versRequired, 
                "A newer version of the \"" + context.session.DeviceMetrics.clientName + "\" client application is required by this endpoint.  Please install the latest version and try again.",
                context.session.DeviceMetrics.clientName
                );
        }
        
        // Check the client version required by the app
        //
        if (this.appDefintion.synchro && this.appDefinition.synchro.clientVersion && !semver.satisfies(context.session.DeviceMetrics.clientVersion, this.appDefinition.synchro.clientVersion))
        {
            throw new ClientVersionError(context.session.DeviceMetrics.clientVersion, this.appDefinition.synchro.clientVersion, 
                "A newer version of the \"" + context.session.DeviceMetrics.clientName + "\" client application is required by the app at this endpoint.  Please install the latest version and try again.",
                context.session.DeviceMetrics.clientName
            );
        }
