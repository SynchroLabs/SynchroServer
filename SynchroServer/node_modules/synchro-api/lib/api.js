...
SynchroApi.prototype.process = function(session, requestObject, responseObject)
{
    ...
    try
    {
        ...
        if (context.request.DeviceMetrics)
        {
            ...
            // !!! Evaluate client version suitability (here for initial request, or perhaps later if we want
            //     to evaluate on every request).
            //
            logger.info("Start session with client: " + context.session.DeviceMetrics.clientName + " - " + context.session.DeviceMetrics.clientVersion);
        }
