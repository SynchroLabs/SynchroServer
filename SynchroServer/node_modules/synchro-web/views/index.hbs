...
<html>
  <head>
    ...
    <style>
      ...
      /* Bootstrap fixed header means we need to pad the body down so it won't be underneath header */
      body {
        padding-top: 60px;
      }
      div.synchro {
      ...
      /* Reduce left/right padding on small screen sizes (Bootstrap) */
  ...
  <body>
    ...
    <nav class="navbar navbar-inverse navbar-fixed-top" role="nagivation">
      <div class="container">
        ...
        <ul class="nav navbar-nav pull-right" style="margin: 0">
          <li>
            <p class="navbar-btn">
              <a id="synchro_back" style="visibility: hidden" class="btn btn-default">Back</a>
            </p>
          </li>
        </ul>
      </div>
    ...
    <script type="text/javascript">
        function SynchroInit(synchro)
        {
            ...
            if (backButton)
            {
                backButton.addEventListener("click", function(){ synchro.goBack(); return false; }, false);
            }

            var endpoint = "{{apiPath}}"; 
            if (!endpoint.match(/^https?:\/\/.*/))
            {
                // Endpoint was not fully specified, so let's build one relative to this page...
                //
