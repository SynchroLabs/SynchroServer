...
// !!! Would be nice if this acted more like a listbox control, in terms of kb support (outlining current item, select/deselect via kb)
//     and also kb modifiers for contiguous selection (shift), item deselection.
...
// Studio editing UX needs containers to work).
//
function getContents(controlSpec, attribute)
{
    var contents = controlSpec[attribute];
    if (Array.isArray(contents))
    {
        contents = contents.length ? contents[0] : null;
    }
    return contents;
}
...
var WebListViewWrapper = function(parent, bindingContext, controlSpec)
{
    ...
    logger.debug("ListView control created");
    ...
    if (controlSpec["header"])
    {
        ...
        this._header = WebControlWrapper.createControl(this, this._bindingContext, getContents(controlSpec, "header"));
    ...
    if (controlSpec["footer"])
    {
        ...
        this._footer = WebControlWrapper.createControl(this, this._bindingContext, getContents(controlSpec, "footer"));
    ...
    {
        var itemTemplate = getContents(controlSpec, "itemTemplate");
        var itemContent = bindingSpec["itemContent"] || "{$data}";

        this.processElementBoundValue(
            "items",
            bindingSpec["items"],
            function() { return self.getListViewContents(listview) },
            function(value) { self.setListViewContents(listview, self.getValueBinding("items").getBindingContext(), itemTemplate, itemContent) }
            );
    }
...
// ListViewItem
//
var WebListViewItemWrapper = function(parent, bindingContext, itemTemplate, itemContent)
{
    this.base = WebControlWrapper;
    ...
    if (!itemTemplate)
    {
        // If no item template, make one that is equivalent to a "listbox" item (this makes us config/API 
        // compatible with "listbox").
        //
        itemTemplate = { control: "text", value: itemContent, margin: 5 };
    }
...
WebListViewWrapper.prototype.setListViewContents = function(listview, bindingContext, itemTemplate, itemContent)
{
    // listview - DOM elemement
    // bindingContext - BindingContext
    // itemTemplate
    // itemContent - string (for listbox compatability)
    //
    ...
    var listItems = []; // Old-timey browsers like IE 10/11 don't support Array.from
    for (var i = 0; i < listview.childNodes.length; i++)
    {
        listItems.push(listview.childNodes[i]);
    }
    ...
    if (listItems.length < itemContexts.length)
    {
        ...
        for (var index = listItems.length; index < itemContexts.length; index++)
        {
            ...
            var itemControl = new WebListViewItemWrapper(this, itemContexts[index], itemTemplate, itemContent);
    ...
    else if (listItems.length > itemContexts.length)
    {
        ...
        for (var index = listItems.length; index > itemContexts.length; index--)
        {
            ...
            //
            var pos = this._childControls.indexOf(wrapper);
            this._childControls.splice(pos, 1);
