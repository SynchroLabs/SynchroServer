...
{
    // We set the CSS class for the font face type.  This allows the app developer to apply their font(s) of
    // choice via CSS.
    //
    this._element.classList.remove("synchro-sanserif");
    this._element.classList.remove("synchro-serif");
    this._element.classList.remove("synchro-monospace");

    faceType = faceType || FontFaceType.FONT_DEFAULT;
    switch (faceType)
    {
        case FontFaceType.FONT_DEFAULT: // on iOS "default" is the system font, on Win it's the same as sanserif.
        case FontFaceType.FONT_SANSERIF:
            this._element.classList.add("synchro-sanserif");
            break;
        case FontFaceType.FONT_SERIF:
            this._element.classList.add("synchro-serif");
            break;
        case FontFaceType.FONT_MONOSPACE:
            this._element.classList.add("synchro-monospace");
            break;
    }
}
...
var WebControlWrapper = function(param1, param2, param3, param4, param5)
{
    ...
    //
    // WebControlWrapper = function(pageView, stateManager, viewModel, bindingContext, control)
    // WebControlWrapper = function(parent, bindingContext, controlSpec)
    //
    var pageView = param1;
    var stateManager = param2;
    var viewModel = param3;
    var bindingContext = param4;
    var control = param5;
...
WebControlWrapper.prototype.processThicknessProperty = function(controlSpec, attributeName, property, control, setValue)
{
    ...
    setValue = setValue || function(property, value) { control.style[property] = self.toDeviceUnits(value) };
...
// Note: On other platforms, we have framework "default" settings, which typically include a default margin (and possibly
//       other default layout values).  For the web client, this is handled in CSS.
//
...
WebControlWrapper.prototype.setHeight = function(value)
{
    var heightString = this.toString(value);
    if (heightString.indexOf("*") >= 0)
    {
        logger.debug("Got star height string:", heightString);
        this._starHeight = ControlWrapper.getStarCount(heightString);
        this._control.style.height = "100%"; // If only control in non-Flexbox div (flexbox will override)
    }
    else
    {
        this._control.style.height = this.toDeviceUnits(value);
    }
    this._heightSpecified = true;
}

WebControlWrapper.prototype.setWidth = function(value)
{
    var widthString = this.toString(value);
    if (widthString.indexOf("*") >= 0)
    {
        logger.debug("Got star width string:", widthString);
        this._starWidth = ControlWrapper.getStarCount(widthString);
        this._control.style.width = "100%"; // If only control in non-Flexbox div (flexbox will override)
    }
    else
    {
        this._control.style.width = this.toDeviceUnits(value);
    }
...
WebControlWrapper.prototype.processCommonFrameworkElementProperies = function(controlSpec)
{
    ...
    // Store the initial (visible) "display" value, so that if we have to toggle visibility later, we know how to toggle it back on...
    //
    self._control_visible_display = (self._control && self._control.style && self._control.style.display) || 'block';
    ...
    // CSS classes specified on control are propogated to HTML element
    //
    this.processElementProperty(controlSpec, "class", function(value) 
    {
        var classes = self.toString(value).split(/[ ,]/g);
        for (var i = 0; i < classes.length; i++)
        {
            self._control.classList.add(classes[i]); 
        }
    });
    ...
    this.processElementProperty(controlSpec, "height", function(value) { self.setHeight(value) });
    this.processElementProperty(controlSpec, "width", function(value) { self.setWidth(value) });

    this.processElementProperty(controlSpec, "minheight", function(value) { self._control.style.minHeight = self.toDeviceUnits(value) });
    this.processElementProperty(controlSpec, "minwidth", function(value) { self._control.style.minWidth = self.toDeviceUnits(value) });
    this.processElementProperty(controlSpec, "maxheight", function(value) { self._control.style.maxHeight = self.toDeviceUnits(value) });
    this.processElementProperty(controlSpec, "maxwidth", function(value) { self._control.style.maxWidth = self.toDeviceUnits(value) });
    ...
    {
        self._control.style.display = self.toBoolean(value) ? self._control_visible_display : 'none';
    });
    ...
    this.processElementProperty(controlSpec, "color", "foreground", function(value) 
    { 
...
{
    return navigator.userAgent.match(/(Android|iPod|iPhone|iPad)/) && navigator.userAgent.match(/AppleWebKit/)
}

WebControlWrapper.createControl = function(parent, bindingContext, controlSpec)
{
    logger.debug("Creating control:", controlSpec["control"]);
    var controlWrapper = null;

    if (isMobileWebKit())
    {
        // WebKit-based browers on WebKit have a number of quirks.  A big one is that they don't support the <select>
        // element with a size attribute or multiple selection (as used by our "listbox" control).  So if we encounter
        // a "listbox" in this environment, we substitue a "listview" (which is API/config compatible, and works fine).
        //
        if (controlSpec["control"] == "listbox")
        {
            controlSpec["control"] = "listview";
        }
    }
    ...
    switch (controlSpec["control"])
    {
        ...
        case "toggle":
            ...
            break;
        case "togglebutton":
            var WebToggleButtonWrapper = require('./web-toggle-button-wrapper');
            controlWrapper = new WebToggleButtonWrapper(parent, bindingContext, controlSpec);
            break;
        case "webview":
