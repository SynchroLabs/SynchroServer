...
var WebStackPanelWrapper = function(parent, bindingContext, controlSpec)
{
    ...
    this.processElementProperty(controlSpec, "orientation", function(value) 
    {
        ...
        for (var i = 0; i < self._childControls.length; i++)
        {
            ...
            //
            logger.info("Processing star sizing based on orientation");
            if (orientation == Orientation.Horizontal)
            {
                if (child._starHeight)
                {
                    child._control.style.height = null;
                    child._control.style.alignSelf = "stretch"; // Perpendicular to layout direction
                }
                if (child._starWidth)
                {
                    child._control.style.width = null;
                    child._control.style.flex = child._starWidth; // In layout direction
                }
            }
            else
            {
                ...
                {
                    child._control.style.height = null;
                    child._control.style.flex = child._starHeight; // In layout direction
                }
            }
