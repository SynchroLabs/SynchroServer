...
var WebSliderWrapper = function(parent, bindingContext, controlSpec)
{
    ...
    logger.debug("Edit control created");
    ...
    if (!this.processElementBoundValue("value", bindingSpec["value"], function() { return new JToken(slider.value) }, function(value) { setSliderValue(slider, self.toDouble(value)) }))
    {
        this.processElementProperty(controlSpec, "value", function(value) { setSliderValue(slider, self.toDouble(value)) });
    }
...
{
    // In the event that the value gets set before the max gets set, it appears that some browsers limit the value to the
    // max at the time the value is set.  So we'll just always make sure the max is at least as much as the value when we 
    // set it to be safe (this problem was observed on Chrome).
    //
    if (slider.max < value)
    {
        slider.max = value;
