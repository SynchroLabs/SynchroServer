...
StateManager.prototype.processRequestFailure = function(request, exception) // JToken / Error?
{
    ...
    this.messageBox("Connection Error", "Error connecting to application server", "Retry", "retry", function(command)
    {
        ...
        _transport.sendMessage(_app.getSessionId(), request);
    });
}
...
StateManager.prototype.processResponseAsync = function(responseAsJSON) // JToken
{
    ...
    if (responseAsJSON["Error"] != null)
    {
        ...
        if (jsonError["name"] == "SyncError")
        {
            if (responseAsJSON["InstanceId"] == null)
            {
                ...
                messageBox("Synchronization Error", "Server state was lost, restarting application", "Restart", "restart", function (command)
                {
                    logger.warn("Corrupt server state, restarting application...");
                    this.sendAppStartPageRequestAsync(); // !!! was await
                });
            ...
            else
            {
                ...
                this.sendResyncInstanceRequestAsync(); // !!! was await
            }
        }
        else
        {
    ...
    else if (responseAsJSON["ViewModel"] != null) // ViewModel without View (resync)
    {
        ...
        else
        {
            ...
            this.sendResyncRequestAsync(); // !!! wa await
            return;
        }
    }
    ...
    if (responseAsJSON["MessageBox"] != null)
    {
        ...
        this._onProcessMessageBox(jsonMessageBox, function (command)
        {
            ...
            this.sendCommandRequestAsync(command); // !!! was await
        });
    }
    ...
    if (responseAsJSON["NextRequest"] != null)
    {
        ...
        this._transport.sendMessage(this._app.SessionId, requestObject); // !!! was await
    }
    else if (updateRequired)
    {
        logger.debug("Sending pending viewModel updates (after request processing)");
        this.sendUpdateRequestAsync(); // !!! was await
    }
}

StateManager.prototype.startApplicationAsync = function() // !!! async
{
    ...
    this._transport.sendMessage(null, requestObject); // !!! was await
}

StateManager.prototype.sendAppStartPageRequestAsync = function() // !!! async
{
    ...
    this._transport.sendMessage(this._app.getSessionId(), requestObject); // !!! was await
}

StateManager.prototype.sendResyncInstanceRequestAsync = function() // !!! async
{
...
logger.info("vmDelta[%s]:", keys[i], JSON.stringify(vmDeltas[keys[i]].getValue(), null, 4));

            deltas.push(
            {
                "path": keys[i],
                "value": vmDeltas[keys[i]].getValue() // !!! Was .deepClone (assuming JToken)
            });
        }
...
StateManager.prototype.sendUpdateRequestAsync = function() // !!! async
{
...
StateManager.prototype.sendCommandRequestAsync = function(command, parameters) // !!! async
{
...
StateManager.prototype.sendBackRequestAsync = function() // !!! async
{
...
StateManager.prototype.sendViewUpdateAsync = function(orientation) // !!! async
{
...
//
StateManager.prototype.sendResyncRequestAsync = function() // !!! async
{
