...
//
ControlWrapper.prototype.toDeviceUnits = function(value)
{
    if (value instanceof JToken)
    {
        value = this.toDouble(value);
    }
    return this._stateManager._deviceMetrics.synchroUnitsToDeviceUnits(value);
}

ControlWrapper.prototype.toFontSizeFromTypographicPoints = function(value)
{
    if (this._stateManager != null)
    {
        return this._stateManager._deviceMetrics.typographicPointsToFontSize(this.toDouble(value));
    }
    else
    {
        // For test cases where we don't have a StateManager, just return the raw size
        return this.toDouble(value) + "pt";
    }
}
...
ControlWrapper.prototype.processFontAttribute = function(controlSpec, fontSetter)
{
    ...
    this.processElementProperty(controlSpec, "font.size", "fontsize", function(value)
    {
        if (value != null)
        {
            fontSetter.setSize(self.toFontSizeFromTypographicPoints(value));
        }
    });
...
ControlWrapper.prototype.processElementProperty = function(controlSpec, attributeName, param3, param4)
{
    ...
    // Convert to JToken so we can call selectToken below to extract the attribute value (selectToken supports
    // dot-separated paths, for things like font.size)
    //
