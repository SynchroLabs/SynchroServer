...
var WebPageView = function(stateManager, viewModel, page, contentControl, doMessageBox) //  page is "document" and contentControl is div container for app
{
    this.base = PageView;
    this.base(stateManager, viewModel, null); // Web does not support doBackMenuCallback

    this.doMessageBox = doMessageBox;
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                logger.info("Popping back to previous page via go(%d) for path: %s, navStack: %s", diff, path, JSON.stringify(this._navStack));
...
WebPageView.prototype.setContent = function(content) // content is ControlWrapper
{
    ...
    // Scroll back to the top for the new page/content...
    //
    // Note: If the navigation was via the broswer back, then the browser will restore the scroll state (there doesn't seem
    //       to be  any way to prevent/override that).  Whereas when we nav using our own back function (or otherwise back 
    //       nav based on server logic), the scroll to top below works properly.  It's a little weird that there are two
    //       different behaviors, but kind of a pain to align them (we could store and restorce scrollTop using the history
    //       state I suppose if we really cared).
    //
    window.scrollTo(0, 0);
...
WebPageView.prototype.processMessageBox = function(messageBox, onCommand) // onCommand is CommandHandler
{
    ...
    //     options: // optional - 0 or more action buttons
    //     [
    //         { label: "Button lable", command: "commandToRun" }
    //     ]
    // }

    // Call client host messageBox handler if provided, else fallback to browser messageBox
    //
    if (this.doMessageBox)
    {
        this.doMessageBox(messageBox, onCommand);
        return;
    }
    ...
    // buttons.  This is not that.
    //
    var title = messageBox["title"]; 
    ...
    if (messageBox.options && messageBox.options.length > 1)
    {
        // Multiple options
        //
        // Confirm will give us "Ok" and "Cancel" and those may or may not map to the first two commands.  This is a great reason
        // that this messageBox implementation should always be overriden by the client host.
        //
        if (confirm((title ? title + "\n\n" : "") + message))
        {
            ...
            {
                onCommand(messageBox.options[0].command);
            }
        }
        else
        {
            ...
            {
                onCommand(messageBox.options[1].command);
            }
        }
    }
    else
...
{
    // There isn't really a cross-browser way to tell what schemes might be supported.  
    //
    var basicUrl = /^(https?)|(ftp)|(mailto):/i;

    if (basicUrl.test(primaryUrl))
    {
        logger.info("Launch primary URL of basic type:", primaryUrl);
        window.open(primaryUrl, '_blank');
    }
    else if (secondaryUrl && basicUrl.test(secondaryUrl))
    {
        logger.info("Launch secondary URL of basic type:", secondaryUrl);
        window.open(secondaryUrl, '_blank');
    }
    else
    {
        // If we didn't have primary or secondary URL of a basic type that we know is supported, then we'll go ahead and try
        // the primaryUrl.  The Synchro app running on the server can detetmine that this is the Web client, and has access 
        // to the UserAgent value, if they want to try to be smart about what schemes they send down.
        //
        logger.info("Launch primary URL of comlpex type:", primaryUrl);
        window.open(primaryUrl, '_blank');
    }
}
