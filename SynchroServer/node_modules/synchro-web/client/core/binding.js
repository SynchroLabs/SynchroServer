    ...
    if (bindingSpec != null)
    {
        if (typeof bindingSpec === 'object')
        {
            ...
            if (defaultAttributeIsCommand && (bindingObject["command"] != null))
            {
                ...
                //
                bindingObject = { [defaultBindingAttribute]: bindingObject };
            }
        }
        else
        {
        ...
        if (commandAttributes != null)
        {
            for (var i = 0; i < commandAttributes.length; i++)
            {
                ...
                //
                if (typeof bindingObject[commandAttribute] !== 'object')
                {
        ...
        logger.debug("Found binding object: %s", bindingObject);
    }
    else
    {
        // No binding spec
...
var BoundAndPossiblyResolvedToken = function(bindingContext, oneTime, negated, formatSpec)
{
    ...
    if (oneTime)
    {
        ...
        //
        this._resolvedValue = this._bindingContext.getValue().deepClone();
        if (this._negated)
        {
...
{
    return PropertyValue.expand(tokenString, bindingContext).toString(); // ???
}
