...
module.exports = function(params)
{
    ...
    {
        return yield function(done){getAppDefinitionAsync(container, done)}
    }
    ...
    var moduleStore = 
    {
        ...
        {
            var containers = yield function(done){getContainerListAsync(done)}
            var synchroAppContainers = [];
            for (var i = 0; i < containers.length; i++)
            {
                var appDefinition = yield getAppDefinition(containers[i]);
                if (appDefinition)
                {
                    if (isSynchroApp(appDefinition))
                    {
        ...
        {
            return yield function(done){createAppContainerAsync(container, done)}
        },

        deleteAppContainerAwaitable: function * (container)
        {
            if (yield function(done){doesContainerExistAsync(container, done)})
            {
                var appDefinition = yield getAppDefinition(container);
                if (appDefinition)
                {
                    if (isSynchroApp(appDefinition))
                    {
                        return yield function(done){deleteContainerAsync(container, done)}
                    }
                    else
                    {
                        // A package.json was found, but not one for a Synchro app...
        ...
        getAppModuleStoreAwaitable: function * (container)
        {
            ...
            var app =
            {
                ...
                {
                    return yield getAppDefinition(container);
                },
                ...
                listModulesAwaitable: function * ()
                {
                    ...
                    var blobs = yield function(done){listBlobsAsync(container, done)}
                ...
                {
                    var content = yield function(done){getBlobTextAsync(container, moduleFilename, done)}
                    return stripBom(content);
                },

                putModuleSourceAwaitable: function * (moduleFilename, content)
                {
                    return yield function(done){setBlobTextAsync(container, moduleFilename, content, done)}
                },

                removeModuleSourceAwaitable: function * (moduleFilename)
                {
                    return yield function(done){removeBlobAsync(container, moduleFilename, content)}
                }
            }
