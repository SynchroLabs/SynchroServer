...
function updateDebugging(isDebugging)
{
    ...
    if (debugging)
    {
        $("#mainToolbar").removeClass("notdebugging").addClass("debugging");
    }
    else
    {
        $("#mainToolbar").removeClass("debugging").addClass("notdebugging");
...
{ 
    ...
    websocket = new WebSocket(proto + window.location.host + path); 
...
function onMessage(evt) 
{
    ...
    switch (event.event)
    {
        ...
        case "break":
        {
            ...
            {
                $('#argumentsTable').append('<tr><td>' + currentFrame.arguments[i].name + '</td><td>' + escapeHtml(JSON.stringify(currentFrame.arguments[i].display)) + '</td></tr>');
            }

            $("#localsTable").empty();
            for (var i = 0; i < currentFrame.locals.length; i++)
            {
                $('#localsTable').append('<tr><td>' + currentFrame.locals[i].name + '</td><td>' + escapeHtml(JSON.stringify(currentFrame.locals[i].display)) + '</td></tr>');
            }
