...
//
// Color pickers (future): (neither support automatic #AARRGGBB)
//
//     https://itsjavi.com/bootstrap-colorpicker/ (no swatch support)
//     http://labs.abeautifulsite.net/jquery-minicolors/ (no support for "Red", "Blue", etc)
//
toastr.options = 
{
    closeButton: true,
...
var controlMeta = 
{
    ...
    "listview": {
        ...
        // !!! This "container" contains an object (single control) rather than an array (array of controls) maybe type: "object" / "array" (defaults to array), or array, singleArray, object (instead of "single" attr)?
    ...
    "toggle": {
        ...
        attributes: [
            { caption: "Caption", attribute: "caption", type: "string" },
            // !!! font - ??? - Does button have font also?
...
function onDocumentReady(modulePathPrefix, webAppPath)
{
    currentModulePathPrefix = modulePathPrefix;
    currentWebAppPath = webAppPath;
    ...
    layout = $('body').layout(
    {
        ...
        center__childOptions: {
            ...
            center__childOptions: {
                ...
                north__paneSelector:  ".editor-north",
                north__size: 300, 
    ...
    editorLayout = $("div.middle-center").layout();
...
function selectViewNodeFromIndexes(indexes) // !!! selectControlFromIndexes?
{
    var controlNode;
    var indexesCopy = indexes.slice(0); // We don't want to modify passed-in indexes array
    var indexesNew = [];

    // console.log("selectViewNodeFromIndex:", indexesCopy);

    try
    {
        var controlNode = getNodeProperty(getViewNode(), 'elements').value;
        ...
        if (controlNode.elements.length > 1)
        {
            ...
            // 
            indexesNew.push(indexesCopy.pop()); 
        }
        ...
        while (controlNode && indexesCopy.length)
        {
            if (controlNode.type === 'ObjectExpression')
            {
                ...
                var containerProps = getContainerProperties(controlType);
                if (containerProps && containerProps.attribute)
                {
                    // console.log("Promoting contents of ObjectExpression");
                    controlNode = getNodeProperty(controlNode, containerProps.attribute).value;
                    if (containerProps.contextAttribute)
                    {
                        // If there is a contextAttribute, then we need to pop an index to simulate navigating into the first element of the context
                        // contents array.
                        //
                        // console.log("Popped for contextAttribute");
                        indexesNew.push(indexesCopy.pop()); 
                    }

                    if (controlNode.type === 'ObjectExpression')
                    {
                        // Normally the contents of the container is an array.  In the case that it's an object (as with listview itemTemplate), 
                        // then we need to pop an index to correspond to navigating into the object, then we continue with the loop (skipping 
                        // the array processing/nav below).
                        //
                        // We also pop this index from indexesCopy without pushing it to indexesNew, because we don't want this index in the result.
                        //
                        // console.log("Popped for ObjectExpression contents");
                        indexesCopy.pop();
                        continue;
                    }
                }
            ...
            if (controlNode.type === 'ArrayExpression')
            {
                ...
                var index = indexesCopy.pop();
                controlNode = controlNode.elements[index];
                indexesNew.push(index);
            }
            else
            {
        ...
        if (controlNode)
        {
            controlNode.indexes = indexesNew; // Reversed copy of original indexes used to find this element (skipping any context selection index elements)
        }
    }
    catch (e)
    {
...
function getSynchroControlFromPoint(x, y)
{
    ...
    {
        if ($(elements[i]).is('[class*="synchro-"]') && !$(elements[i]).is('[class*="synchro-listview-item"]'))
        {
...
function findControl(node, test, indexer)
{
    ...
    if (arrayToProcess)
    {
        for (var i = 0; i < arrayToProcess.length; i++)
        {
            ...
            {
                theIndexer.currIndex = 0; // We don't want property index
            }
            else // Array (or Body)
            {
...
function selectVisualElementFromIndexes(visualElement, indexes)
{
    ...
    // If we're in a synchro-listview, we want to select the first synchro-listview-item child, then select
    // the visual element at the next index from there.  We skip the synchro-listview-item because it 
    // does not represent a control, and we select the first child in order to make our selection within
    // that first child (which essentially serves as the visual prototype of all items in the list).
    //
    var items = $visualElement.children('[class*="synchro-listview-item"]');
    if (items && items.length)
    {
        $visualElement = $(items.get(0));
    }
...
function populatePropertyInspectorWidgets(divId, properties, isBindings)
{
    if (properties && properties.length)
    {
        for (var i = 0; i < properties.length; i++)
        {
            ...
            $('#widget-' + property.attribute).data('isBinding', isBindings);
        }
...
function getFirstBindingPathElement(bindingPath)
{
    var match = bindingPath.match(/^([^.]+)[.].*/);
    return (match && match[1]);
}

function getRelativeBindingPath(controlBindingPath, bindingPath)
{
    if (controlBindingPath && (controlBindingPath !== bindingPath))
    {
        // If bindingPath doesn't have a prefix, then make bindingPath $root based
        // If bindingPath has a prefix and it doesn't match the controlBindingPath prefix, make bindingPath $root based
        //
        var firstBindingPathElement = getFirstBindingPathElement(bindingPath);
        var firstControlBindingPathElement = getFirstBindingPathElement(controlBindingPath) || controlBindingPath;
        if (!firstBindingPathElement || (firstBindingPathElement !== firstControlBindingPathElement))
        {
            // If we don't share command ancestry (other than root), then we'll make this a $root binding (it doesn't make sense
            // to navigate up with a bunch of $parent.$parent.$parent just to get to the root).  This models what a sane developer
            // would typically do manually in this situation.
            //
    ...
    var bindingPrefix = "";
    while (controlBindingPath)
    {
        if (bindingPath === controlBindingPath)
        {
            // bindingPath and controlBindingPath match - we have to take one step up to get from inside of binding context to binding context
            //
            bindingPath = "$parent";
            controlBindingPath = ""; // break
        }
        else if (bindingPath.indexOf(controlBindingPath + ".") === 0)
        {
            // bindingPath starts with controlBindingPath - remove controlBindingPath prefix from bindingPath
            //
            bindingPath = bindingPath.slice(controlBindingPath.length + 1);  // Remove bindingPath prefix and dot
            bindingPath = bindingPath.replace(/^[0-9]*[.]?/, "");            // Remove array index prefix
            if ((bindingPrefix.length === 0) && (bindingPath.length === 0))
            {
                bindingPath = "$data";
            }
            controlBindingPath = ""; // break
        ...
        {
            // Back up one on controlBindingPath - remove last path element from controlBindingPath, add $parent to bindingPrefix, spin again
            //
            controlBindingPath = controlBindingPath.replace(/[.]?[^.]*$/, "");
            if (!bindingPrefix)
            {
                // If this is the first time we're backing up, we need to do it twice to get from the item, to the list, then above the list (that we're removing).
                //
                bindingPrefix += "$parent.";
            }
            bindingPrefix += "$parent.";
        }
    }

    return bindingPrefix + bindingPath;
}

function getBindingSpecForControlType(controlType, attribute)
{
    console.log("Getting binding spec for control type: %s and attribute: %s", controlType, attribute);

    var controlProps = controlMeta[controlType];
    if (controlProps && controlProps.bindings)
    {
        for (var i = 0; i < controlProps.bindings.length; i++)
        {
            if (controlProps.bindings[i].attribute === attribute)
            {
                return controlProps.bindings[i]; 
            }
        }
    }
}

function isDefaultBindingAttributeForControlType(controlType, bindingAttribute)
{
    var controlProps = controlMeta[controlType];
    if (controlProps && controlProps.bindings && controlProps.bindings.length)
    {
        return controlProps.bindings[0].attribute === bindingAttribute;
    }
    return false;
}

function getBindingContextValue(controlNode, contextAttribute)
{
    console.log("Getting binding context value for context attribute:", contextAttribute);

    var controlType = getNodePropertyValueAsString(controlNode, "control");

    var bindingProperty = getNodeProperty(controlNode, "binding");
    if (bindingProperty)
    {
        if (bindingProperty.value.type === "ObjectExpression")
        {
            var contextAttributeProperty = getNodeProperty(bindingProperty.value, contextAttribute);
            if (contextAttributeProperty && contextAttributeProperty.value.type === "Literal")
            {
                return contextAttributeProperty.value.value.toString();
            }
        }
        else if (isDefaultBindingAttributeForControlType(controlType, contextAttribute))
        {
            if (bindingProperty.value.type === "Literal")
            {
                return bindingProperty.value.value.toString();
            }
        }
    }
}

function getBindingPathForControl(controlNode, propName, isDataBinding)
{
    console.log("Getting bindings for control:", controlNode);
    ...
    if (propName)
    {
        // Process the property for this node 
        //
        if (isDataBinding) // Only data binding props will actually having a possible binding context (contextAttribute)
        {
            var bindingProps = getBindingSpecForControlType(controlType, propName);
            console.log("Got binding props for control:", bindingProps);
            if (bindingProps && bindingProps.contextAttribute)
            {
                // Get binding value for contextAttribute
                //
                bindingPath = getBindingContextValue(controlNode, bindingProps.contextAttribute);
                if (bindingPath)
                {
                    console.log("Got bindingPath for property:", bindingPath);
                }
                else
                {
                    console.log("Error - contextAttribute specified for control type, but no corresponding binding value found"); // !!!
                }
            }
        }
    }
    ...
    {
        // Process this node as a container
        //
        if (controlMeta[controlType] && controlMeta[controlType].container)
        {
            var containerProps = controlMeta[controlType].container;
            if (containerProps.contextAttribute)
            {
                // Get binding value for contextAttribute
                //
                bindingPath = getBindingContextValue(controlNode, containerProps.contextAttribute);
                if (bindingPath)
                {
                    console.log("Got bindingPath for container:", bindingPath);
                }
                else
                {
                    console.log("Error - contextAttribute specified for control type, but no corresponding binding value found"); // !!!
        ...
        {
            console.log("Error - Control type not found in metadata, or control type not a container:", controlType); // !!!
        }
    }

    if (controlNode.parent)
    {
        var parentBindingPath = getBindingPathForControl(controlNode.parent);
        if (parentBindingPath)
        {
            if (bindingPath)
            {
                bindingPath = parentBindingPath + "." + bindingPath;
            }
            else
            {
                bindingPath = parentBindingPath;
            }
        }
    }
...
function onBinding(propName, isDataBinding)
{
    ...
    var controlBindingPath = getBindingPathForControl(selectedControl, propName, $control.data('isBinding'));
    if (controlBindingPath)
    {
        $('#baseBindingPath').html(controlBindingPath);
        $('#modalPropValue').data('controlBindingPath', controlBindingPath);
        $('#baseBindingPathGroup').show();
    }
    else
    {
        $('#modalPropValue').removeData('controlBindingPath');
        $('#baseBindingPathGroup').hide();
    }
...
function onBindingViewModelClick(pos)
{
    ...
    if (bindingPath)
    {
        var controlBindingPath = $('#modalPropValue').data('controlBindingPath');

        var relativeBindingPath = getRelativeBindingPath(controlBindingPath, bindingPath);
        console.log("onBindingViewModelClick - controlBindingPath: %s, bindingPath: %s, relativeBindingPath: %", controlBindingPath, bindingPath, relativeBindingPath);

        var isDataBinding = $('#modalPropValue').data('isDataBinding');
        if (isDataBinding)
        {
            // For value binding, we replace the value with the relative binding path
            //
            $('#modalPropValue').val(relativeBindingPath);
        }
        else
        {
            ...
            $('#modalPropValue').val(selection.val.substring(0, selection.start) + "{" + relativeBindingPath + "}" + selection.val.substring(selection.end));
...
{
    editor.setReadOnly(isDebug);

    if (isDebug)
    {
        $('#synchro-web-app').attr('src', currentWebAppPath); // Reload Synchro web app

        $('#save').hide();
        layout.hide("east");         // Hide UX tools
        editorLayout.hide("north");  // Hide UX Preview
        editorLayout.show("south");  // Show Debugging details
        centerLayout.show("east");   // Show Synchro web app
    }
    else
    {
        centerLayout.hide("east");   // Hide Synchro web app
        editorLayout.hide("south");  // Hide Debugging details
        editorLayout.show("north");  // Show UX Preview
        layout.show("east");         // Show UX tools
        if ($("#module").text())
        {
            $('#save').show();
        }
...
function onNewModule()
{
    ...
    {
        bootbox.confirm("There are unsaved changes to '" + currentModule + "'.  <br/><br/>Do you want to continue creating a new module, abandoning those changes?", function(result)
        {
            if (result)
            {
                newModule();
...
function onLoadModule(moduleName)
{
    if (!editor.session.getUndoManager().isClean())
    {
        bootbox.confirm("There are unsaved changes to '" + currentModule + "'.  <br/><br/>Do you want to load the new module '" + moduleName + "', abandoning those changes?", function(result)
        {
            if (result)
            {
...
function onDebugStart(port)
{
    if (!editor.session.getUndoManager().isClean())
    {
        bootbox.confirm("There are unsaved changes to '" + currentModule + "'.  <br/><br/>Do you want to start debugging anyway?", function(result)
        {
            if (result)
            {
                debugStart(port);
