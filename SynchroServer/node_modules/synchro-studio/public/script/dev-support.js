...
function initEditor()
{
    ...
    //
    // Deterine the orientaton of the nearest container (if we're over a non-container control, this will be that control's
    // parent, and if we are over a container control, it will be that container control).
    //
...
function getUXDropTarget(clientX, clientY)
{
    ...
    if (container) // We are over a container, position relative to container contents.
    {
        ...
        {
            var beforeElement = -1;
            var orientation = getOrientation(container.get(0));

            for (var i = 0; i < children.length; i++)
            {
                if (isPositionBefore(children[i], orientation, clientX, clientY))
                {
                    beforeElement = i;
                    break;
                }
    ...
    {
        var parentOrientation = "Horizontal";
        var parent = $(synchroControl).parent();
        if (parent && (parent.length > 0))
        {
            // flexDirection "column" is vertical, "row" is horizontal (default)
            //
            parentOrientation = getOrientation(parent.get(0));
        }

        target.position = isPositionBefore(synchroControl, parentOrientation, clientX, clientY) ? "before" : "after";
        target.attribute = null;
    }
...
function showUXDropTarget(target)
{
    ...
    // !!! This needs lots of work.  We need to consider the position (before/after/into).  For before/after, we aren't going to
    //     just highlight the target, but need to place a placeholder element before or after the target.  And we need to track
    //     this placeholder so we can remove it on clearUXDropTarget().  For "into" (which should only be empty containers), we can
    //     just highlight the target.
    //
