...
// !!! Currently called whenever cursor moves (which captures some, but not all, changes)
//
function updateProperties() // onEditorChange - update property inspector values from node (if cursor in control node).  
{
    // Note: If we aren't in a control node, then we shouldn't have an property inspector widgets.
    //
    var pos = editor.selection.getCursor();
    controlNode = findControlInModule(pos.row + 1, pos.col);
