...
function getEditorDropTarget(docPos)
{
    ...
    if (view && elements && nodeContainsPos(elements, docPos))
    {
        ...
        if (contentsArrayNode && (contentsArrayNode.type === 'ArrayExpression'))
        {
            ...
            {
                var controlType = getNodePropertyValueAsString(target.node, "control");
...
function getUXDropTarget(clientX, clientY)
{
    ...
    else // We are not over a container, position relative to control we are over.
    {
        ...
        if (parent && (parent.length > 0))
        {
            ...
            if (parentNode)
            {
                var controlType = getNodePropertyValueAsString(parentNode, "control");
                if ((controlType === 'border') || (controlType === 'scrollview'))
                {
...
function onBinding(propName, isDataBinding)
{
    ...
    {
        // !!! Propety binding can have negation/one-time modifiers (maybe via check box, only enabled when cursor inside token in edit control)
...
// Given a property value (value) that may contain tokens, and a selection range, determine if the selection
// contains, or is entirely within, a token, and if so, return details on the matched token.
//
function getSelectedTokenString(value, selStart, selEnd)
{
    if (selStart != selEnd)
    {
        var sel = value.substring(selStart, selEnd);
    ...
    while ((match = tokenRe.exec(value)) != null) 
    {
        ...
        if ((selStart >= match.index) && (selStart < matchEnd) && (selEnd <= matchEnd))
        {
...
function onBindingViewModelClick(pos)
{
    ...
    if (bindingPath)
    {
        ...
        else
        {
            ...
            //
            var start = $('#modalPropValue').prop('selectionStart');
            var end = $('#modalPropValue').prop('selectionEnd');
            var val = $('#modalPropValue').val();
            console.log("Selection start: %d, end: %s, val: %s", start, end, val);

            var token = getSelectedTokenString(val, start, end);
            if (token)
            {
                // Replace token instead of selection
                console.log("Found token in selection:", token);
                start = token.start;
                end = token.end;
            }

            $('#modalPropValue').val(val.substring(0, start) + "{" + bindingPath + "}" + val.substring(end));
        }
    }
}
