...
function initEditor()
{
    ...
    // !!! It would be cool to show the insertion point on both the UX preview and the editor when dragging, regardless of
    //     which target the control is being dragged over.
    //
    ...
    //
    // !!! To show insertion point, if we want to support not just left/right of element, but also above/below, then it
    //     seems like we have to insert an actual element in the target position (::before and ::after only do left/right).
    //
    ...
    $("#synchro_container").on('dragover', function(ev)
    {
        ...
        // !!! If we are over a container, drop target is container control (where though?)
        //
        // !!! If we are over a control, we find out if our container is vertical or horizontal (flex-flow: row/column)
        //     If mouse is over first half of control (in direction of parent orientation), place the marker element
        //     before the control, else place after.
        //
...
function dropOnEditor(target, data)
{
    if (target.position === "into")
    {
        ...
        if (contentsProp && (contentsProp.value.type === 'ArrayExpression'))
        {
            ...
            var insertedRange = Range.fromPoints(contentValueRange.start, end);
            editor.selection.setSelectionRange(insertedRange);
        }
    }
    else if (target.position === 'before')
    {
        ...
        var insertedRange = Range.fromPoints(range.start, end);
        editor.selection.setSelectionRange(insertedRange);
    }
    else if (target.position === 'after')
    {
        ...
        var insertedRange = Range.fromPoints(range.end, end);
        editor.selection.setSelectionRange(insertedRange);
    }
}
...
function onSelectionChange()
{
    ...
    if (selectedControl != controlNode)
    {
        ...
        if (controlNode)
        {
            ...
            // !!! We probably want to group these attributes (general versus control-specific, maybe other ways) as opposed to just
            //     a giant list.  But for now, we make a giant list...
            //
...
//
// For Ace editor, we can convert screen coords to a document position...
//
//   var screenPos = editor.renderer.pixelToScreenCoordinates(x, y)
//   var docPos = editor.session.screenToDocumentPosition(screenPos.row, screenPos.column)
//
// Then we can look to see if the drop target at that location is a container control (or an item in a container control), and if so,
// position the insertion point appropriately to show where the control would be inserted when dropped.  Should be able to drop into
// empty container, or in any position in a container that has contents (including the beginning or end).
//
