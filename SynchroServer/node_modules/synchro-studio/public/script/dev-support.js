...
var controlMeta = 
{
    ...
    "listbox": {
        ...
        bindings: [
            { attribute: "items", type: "data" } // !!! More (complex)
        ], 
    ...
    "location": {
        ...
        bindings: [
            { attribute: "value", type: "data" }, 
            { attribute: "sync", type: "select", values: ["change"], tokens: false },
            { attribite: "onUpdate", type: "command" }
        ],
        visualElement: false,
...
{
    return getNodePropertyValue(controlNode, property);
}
...
function setControlPropertyValue(controlNode, property, value)
{
    ...
    //
    var attributeIndex = attributeOrder.indexOf(property);
    if (attributeIndex != -1)
    {
        console.log("attributeOrder index of property '%' is:", property, attributeIndex);
        ...
        for (var j = attributeIndex-1; j >= 0; j--)
        {
            ...
            if (precedentIndex != -1)
            {
                ...
                console.log("Add property '%s' after precedent node:", property, precedentAttributeNode);
                ...
                //
                ignoreSelChange = true;
                editor.getSession().insert({row: precedentAttributeNode.loc.end.line-1, column: precedentAttributeNode.loc.end.column}, ", " + property + ": " + value);
                ignoreSelChange = false;
                break;
            }
        }
    ...
    {
        // !!! property attribute not found in attributeOrder array - add the property someplace reasonable (at the end, or right before
        //     "content" if "content" exists).
        //
        console.log("!!! Property '%' not found in attributeOrder array", property);
    }
}

function getControlBindingValue(controlNode, binding)
{
    // !!! Default binding logic is the tricky part of this...
    //
    var binding = getNodeProperty(controlNode, "binding");
    if (binding)
    {
...
{
    // !!! Consider default binding (we may be setting the default binding, or the default binding may already be set and we may
    //     be setting a non-default binding that causes us to have to promote the already set default binding).  Also note that we
    //     may be unsetting a binding if value is empty/null, and that may also result in unsetting all bindings (removing "binding").
    //
    console.log("setControlBindingValue - %s = %s (not implemented yet)", binding, value);
}

function getModuleCommands()
{
    // !!! Get from exports.Commands
}

function createModuleCommand(commandName)
{
    // !!! Create commandName if it doesn't already exist (add to editor).  Consider that exports.Commands itself may need to be
    //     created/added.
}
...
function onEditorChange()
{
    ...
    //     then we need to do the onSelectionChange() logig (which will probably get called anyway - not sure the editor content
    ...
    if (controlNode)
    {
        ...
        attributes = attributes.concat(controlMeta[getNodePropertyValue(controlNode, 'control')].attributes);
        ...
        var bindings = controlMeta[getNodePropertyValue(controlNode, 'control')].bindings;
        if (bindings)
        {
...
{
    // !!! Implement - This is going to be like onBinding (probably just a parameterized version) - show ViewModel and allow clicking
    //     on any element.  Value should not be editable.  No negation/one-time modifiers.
    //
    var propName = $(control).attr("id").match(/^widget-(.*)-binding$/)[1];
    alert("DataBinding dialog for: " + propName);
}
...
function onCommandAccessory(control)
{
    ...
    //
    var propName = $(control).attr("id").match(/^widget-(.*)-binding$/)[1];
    alert("Command dialog for: " + propName);

    // !!! Parse commands from exports.Commands and let user pick one.  Or let them type a new command name.
    //     On completion (when bindingChange is called) if the command does not exist in exports.Commands, a new corresponding command
    //     function should be created there.
}

function onBinding(propName)
{
...
function onBindingSave()
{
    ...
    propChange($('#widget-' + propName).get(0));
}
