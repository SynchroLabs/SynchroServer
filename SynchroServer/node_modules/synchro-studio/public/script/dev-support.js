...
function setControlPropertyValue(controlNode, property, value)
{
    ...
    for (var i = 0; i < controlNode.properties.length; i++)
    {
        ...
        if (prop.key.name == property)
        {
            ...
            updateView(); // !!! We probably either need to do this below as well, or we don't need it either place
            return;
        }
...
function propChange(control, isBinding)
{
    ...
    if (value)
    {
        try
        {
            value = JSON.stringify(JSON.parse(value)); // The point of this is to convert bools, null, and int strings to base types
        }
        catch (e)
        {
            // !!! In the case of a JavaScript object (not a JSON representation), the JSON parse will fail and we'll end up stringifying
            //     the value, which is a potential problem.
            //
            // !!! How do we distinguish {firstName} - {lastName} (token string) from { size: 10, bold: true } (font spec object)?
            //
            value = '"' + value + '"';
        }
    }
