...
SynchroStudio.prototype.addRoutes = function(expressApp, config, checkAuthFn)
{
	...
	expressApp.get('/', function(request, response) 
	{
	    ...
	    co(function * ()
	    {
	        ...
	        for (appPath in apiProcessors)
	        {
	            ...
	            var studioPath = self.getUrlPrefix() ? (self.getUrlPrefix() + "/" + appPath + "/dev") : null;
	...
	{
        var webAppPath = self.webAppUrlPrefix ? (self.webAppUrlPrefix + "/" + req.params.appName) : null;

		co(dev.dev, self, req.params.appName, webAppPath, req, res).catch(function(err)
        {
	...
	expressApp['post'](this.basePath + '/:appName', checkAuth, function(req,res)
	{
	    co(restApi.createApp, self, req.params.appName, req, res).catch(function(err)
        {
            logger.error("Error in restApi.createApp:", err);
        });
	});

	expressApp['get'](this.basePath + '/:appName', checkAuth, function(req,res)
	{
	    co(restApi.getApp, self, req.params.appName, req, res).catch(function(err)
        {
            logger.error("Error in restApi.getApp:", err);
        });
	});
	...
	{
	    co(restApi.createModule, self, req.params.appName, req, res).catch(function(err)
        {
            logger.error("Error in restApi.createModule:", err);
        });
	});

	expressApp['get'](this.basePath + '/:appName/module', checkAuth, function(req,res)
	{
	    co(restApi.loadModule, self, req.params.appName, req, res).catch(function(err)
        {
            logger.error("Error in restApi.loadModule:", err);
        });
	});

	expressApp['put'](this.basePath + '/:appName/module', checkAuth, function(req,res)
	{
	    co(restApi.saveModule, self, req.params.appName, req, res).catch(function(err)
        {
            logger.error("Error in restApi.saveModule:", err);
        });
	});

	expressApp['delete'](this.basePath + '/:appName/module', checkAuth, function(req,res)
	{
	    co(restApi.deleteModule, self, req.params.appName, req, res).catch(function(err)
        {
            logger.error("Error in restApi.deleteModule:", err);
        });
	});
}
